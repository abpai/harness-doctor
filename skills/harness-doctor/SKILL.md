...
```bash
npx harness-doctor@latest --verbose
```
...
1. **Scan.** Run `npx harness-doctor@latest --verbose` and read the full output:
...
5. **Validate.** Re-run `npx harness-doctor@latest --verbose --diff` and confirm
...
`npx harness-doctor@latest rules explain <rule>`, then apply the narrowest
control via `npx harness-doctor@latest rules disable|set|category|ignore-tag …`,
