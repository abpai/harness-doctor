...
- Inherit the latest shared rule registry from `oxlint-plugin-harness-doctor@0.2.10`: Preact compatibility checks, HTML correctness and dialog accessibility rules, `hooks-no-nan-in-deps`, Jotai atom diagnostics, React Native performance rules, `js-async-reduce-without-awaited-acc`, and React 19.2 `<Activity>` effect-boundary checks.
...
- Inherit the `no-mutating-reducer-state` rule and helper consolidation from `oxlint-plugin-harness-doctor@0.2.7`.
...
- Inherit the `jsx-key` shorthand fragment fix, static template literal normalization, and Node 20 support from `oxlint-plugin-harness-doctor@0.2.5`.
...
- Inherit the Effect v4 runtime adoption, deprecated type stub removal, and user-plugin extension support from `oxlint-plugin-harness-doctor@0.2.4`.
...
- Fix build configuration so the ESLint plugin resolves its dependency on `oxlint-plugin-harness-doctor` correctly at publish time.
...
- Inherits the rule-fix wave from
  `oxlint-plugin-harness-doctor@0.2.0-beta.5` via the shared rule
  ...
  fixes ([#265](https://github.com/millionco/harness-doctor/pull/265)),
  ...
  ([#274](https://github.com/millionco/harness-doctor/pull/274)). See
...
- Inherits the `no-barrel-import` index-resolution fix from
  [#253](https://github.com/millionco/harness-doctor/pull/253) via the
...
- Inherits the per-rule module restructuring from
  ...
  ([#249](https://github.com/millionco/harness-doctor/pull/249) and
  follow-ups). The published ESLint plugin shape (flat-config-ready
  `recommended` / framework presets, `harness-doctor/*` rule namespace)
...
- Inherits the beta.2 false-positive sweep from
  ...
  ([#208](https://github.com/millionco/harness-doctor/pull/208)),
  React-19 rule version-gating
  ([#254](https://github.com/millionco/harness-doctor/pull/254)),
  render-reachable state analysis
  ([#255](https://github.com/millionco/harness-doctor/pull/255)),
  narrowed `no-effect-event-handler` detection
  ([#256](https://github.com/millionco/harness-doctor/pull/256)), and
  local `useX` helper suppression + new typography rules
  ([#257](https://github.com/millionco/harness-doctor/pull/257)).
