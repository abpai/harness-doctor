...
// rules now ship inside `harness-doctor`, semantically they ARE the
...
// harness-doctor's distinctive checks) should still not receive them.
...
const ruleLines = ruleEntries
  .map(
    (entry) =>
      `  {\n` +
      `    key: "harness-doctor/${entry.ruleId}",\n` +
      `    id: "${entry.ruleId}",\n` +
