import { harnessDoctorRules } from "./plugin/rule-registry.js";
import type { RuleFramework } from "./plugin/utils/rule.js";
import type { OxlintRuleSeverity } from "./types.js";

type RegistryEntry = (typeof harnessDoctorRules)[number];
...
// config builder in `@harness-doctor/core` honors this flag via the
...
const collectHarnessDoctorRulesByFramework = (frameworkName: RuleFramework) =>
  harnessDoctorRules.filter(
...
export const RULES = [...HARNESS_DOCTOR_RULES, ...EXTERNAL_RULES] as const;

export const RECOMMENDED_RULES = toRuleMap(
  toKeyedSeverity(collectHarnessDoctorRulesByFramework("global")),
);
export const NEXTJS_RULES = toRuleMap(
  toKeyedSeverity(collectHarnessDoctorRulesByFramework("nextjs")),
);
export const REACT_NATIVE_RULES = toRuleMap(
  toKeyedSeverity(collectHarnessDoctorRulesByFramework("react-native")),
);
export const TANSTACK_START_RULES = toRuleMap(
  toKeyedSeverity(collectHarnessDoctorRulesByFramework("tanstack-start")),
);
export const TANSTACK_QUERY_RULES = toRuleMap(
  toKeyedSeverity(collectHarnessDoctorRulesByFramework("tanstack-query")),
);
export const PREACT_RULES = toRuleMap(
  toKeyedSeverity(collectHarnessDoctorRulesByFramework("preact")),
);
export const ALL_HARNESS_DOCTOR_RULES = toRuleMap(toKeyedSeverity(HARNESS_DOCTOR_RULES));
export const ALL_HARNESS_DOCTOR_RULE_KEYS: ReadonlySet<string> = new Set(
  HARNESS_DOCTOR_RULES.map((rule) => rule.key),
);
