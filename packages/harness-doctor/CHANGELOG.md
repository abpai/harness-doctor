...
- [#643](https://github.com/millionco/harness-doctor/pull/643) [`0c8b797`](https://github.com/millionco/harness-doctor/commit/0c8b797d18d7d8d0b347fbea0da111b38075eb5d) Thanks [@rayhanadev](https://github.com/rayhanadev)! - harness-doctor no longer crashes when `git` isn't installed.
...
- [#642](https://github.com/millionco/harness-doctor/pull/642) [`2aa96f3`](https://github.com/millionco/harness-doctor/commit/2aa96f39b56555722b7121569a5bbd9caa10dc44) Thanks [@rayhanadev](https://github.com/rayhanadev)! - Expected, user-actionable failures are no longer reported to Sentry or rendered as crashes.

  When harness-doctor exits because of the user's project or invocation — not a bug — it now prints a clean, single-line message and exits non-zero, instead of the generic "Something went wrong, open a prefilled issue" block. These cases are also no longer sent to Sentry or counted in the alertable error-rate metric. This was flooding crash reporting with non-bugs from CI, coding agents, and sandboxes.
  ...
  - **Read-only config directory** — harness-doctor no longer crashes when it can't create/read its global setup-prompt store on a locked-down or read-only filesystem; it degrades gracefully (REACT-DOCTOR-E).
...
- [#638](https://github.com/millionco/harness-doctor/pull/638) [`114893e`](https://github.com/millionco/harness-doctor/commit/114893e07d83d143aa3cdddc9fb178137b786f47) Thanks [@aidenybai](https://github.com/aidenybai)! - Redesigned the interactive scan report and added a first-run onboarding reveal.
...
- [#637](https://github.com/millionco/harness-doctor/pull/637) [`8162bfb`](https://github.com/millionco/harness-doctor/commit/8162bfb7ca0948b137dd75a22776e55cab99b740) Thanks [@aidenybai](https://github.com/aidenybai)! - Redesign the scan output's summary and footer. The default (non-verbose) run no longer lists every warning rule — warnings are rolled into a single overflow line alongside the hidden errors (e.g. `+4 more rules and +50 optional warnings — run npx harness-doctor@latest --verbose for details`). `--verbose` now renders warnings in the same boxed, titled, code-framed format as errors (with a "Learn more" docs link), instead of a separate compact list. The closing footer is restructured into a `Share:` / `Docs:` / `GitHub:` block (each with a one-line description) separated by a divider, and the share link now appears for monorepo runs too (gated the same way as single-project: shown unless CI, `--no-score`, or `share: false`). The scan spinner's worker count now reads as a dimmed `[~N workers]`.

- [#633](https://github.com/millionco/harness-doctor/pull/633) [`520b0a9`](https://github.com/millionco/harness-doctor/commit/520b0a9235d37074821c12fd73455d8462128a75) Thanks [@rayhanadev](https://github.com/rayhanadev)! - `--diff` now accepts git commit ranges, and a bad `--diff` value is no longer treated as a crash.

  - `--diff A..B` (two-dot, diff A directly against B) and `--diff A...B` (three-dot, diff from the merge-base of A and B to B) are now supported, matching git's own range syntax — an empty endpoint defaults to `HEAD` (`main..` ⇒ `main..HEAD`). Previously any value containing `..` was rejected outright, so `harness-doctor --diff 7694215..c4de712` failed. Each range endpoint is still individually validated against the anti-injection guard, so a range can't smuggle a `--upload-pack=…`-style option past it.
...
- [#635](https://github.com/millionco/harness-doctor/pull/635) [`bd8298d`](https://github.com/millionco/harness-doctor/commit/bd8298d6cf3484ef7f2898fe981442706ffea3ce) Thanks [@rayhanadev](https://github.com/rayhanadev)! - Lint in parallel by default. Harness Doctor now fans the lint pass across your CPU cores out of the box (previously serial) and automatically falls back to a single worker if a parallel run exhausts system resources (`EAGAIN`/`EMFILE`/`ENFILE`/`ENOMEM`); any other failure still surfaces. Pass `--no-parallel` (or set `HARNESS_DOCTOR_PARALLEL=0`) to force serial linting, or set `HARNESS_DOCTOR_PARALLEL=<n>` to pin a worker count. The experimental `--experimental-parallel` flag is replaced by `--no-parallel`.

- [#634](https://github.com/millionco/harness-doctor/pull/634) [`17e722e`](https://github.com/millionco/harness-doctor/commit/17e722ee074d10a5c4082b9f0a6b40ccaf3bed3b) Thanks [@rayhanadev](https://github.com/rayhanadev)! - Add anonymized Sentry Application Metrics (counters + distributions) to the CLI, alongside the existing crash reporting and tracing, so we can track reliability/performance and prioritize work.
  ...
  - **Opt-out unchanged**: `--no-score` (and its `--no-telemetry` alias) disables metrics along with crash reporting and tracing; metrics are skipped under test runs, and the programmatic `@harness-doctor/api` library never initializes Sentry.
...
- [#623](https://github.com/millionco/harness-doctor/pull/623) [`b9e9bcb`](https://github.com/millionco/harness-doctor/commit/b9e9bcbc08985f4bd77df1f354713d2cdbdaf2ec) Thanks [@rayhanadev](https://github.com/rayhanadev)! - Align the CLI with the clig.dev and 12-factor CLI guidelines:

  - `--color` / `--no-color` flags force or disable colored output, with app-specific `HARNESS_DOCTOR_NO_COLOR` / `HARNESS_DOCTOR_FORCE_COLOR` env overrides. Flags win over env vars, which win over picocolors' built-in `NO_COLOR` / `FORCE_COLOR` / `TERM` / TTY detection; the preference is resolved before parsing so it reaches every surface (scan report, branded header, score, prompts, errors).
  - `harness-doctor --help` and `harness-doctor install --help` now lead with worked examples and link to where to report feedback.
  - New `harness-doctor version` subcommand prints the version with Node and platform info (e.g. `harness-doctor/0.2.14 darwin-arm64 node-v24.14.0`); `-v` / `-V` / `--version` stay terse for scripts.
  - `harness-doctor help` and `harness-doctor help <command>` now show help instead of failing by trying to scan a directory named "help".

- [#583](https://github.com/millionco/harness-doctor/pull/583) [`4bc8a73`](https://github.com/millionco/harness-doctor/commit/4bc8a73d247d9b8378616e738780af3794f0b97f) Thanks [@aidenybai](https://github.com/aidenybai)! - Port expo-doctor's project-level checks as Expo-gated diagnostics. When an Expo project is detected (`expoVersion !== null`), harness-doctor now runs the statically-determinable subset of expo-doctor's check suite during the environment-checks phase (skipped in diff/staged mode):
  ...
  The remaining expo-doctor checks require running the Expo CLI, querying the Expo API, or inspecting native iOS/Android projects — none of which fit harness-doctor's offline, static model — so they're intentionally out of scope.

- [#583](https://github.com/millionco/harness-doctor/pull/583) [`4bc8a73`](https://github.com/millionco/harness-doctor/commit/4bc8a73d247d9b8378616e738780af3794f0b97f) Thanks [@aidenybai](https://github.com/aidenybai)! - Detect Expo projects independently of the single-valued `framework` hint. Project discovery now surfaces an `expoVersion` signal (the declared `expo` package spec, looked up in the project or any of its workspace packages, or `null`), paralleling `reactVersion`. The `expo` capability is keyed off `expoVersion !== null` rather than `framework === "expo"`, so Expo-specific rules now load on web-rooted monorepos whose `apps/mobile` workspace targets Expo, and on projects that declare both `expo` and a web bundler (where `vite` / `next` previously won framework detection and silently dropped the `expo` capability). The file-level package boundary in `oxlint-plugin-harness-doctor` still keeps Expo-only rules quiet on web workspaces.

- [#615](https://github.com/millionco/harness-doctor/pull/615) [`8b313ba`](https://github.com/millionco/harness-doctor/commit/8b313badda74de19ba56a242d965c54399d39b9c) Thanks [@rayhanadev](https://github.com/rayhanadev)! - Fix two dead-code / fix-recipe papercuts surfaced on alias-heavy Next.js projects.
  ...
  **Per-rule fix-recipe URLs are only shown when a recipe exists.** Findings advertised a "fetch the canonical fix recipe" URL (`/prompts/rules/<plugin>/<rule>.md`) for every diagnostic, but recipes are only published for harness-doctor's own engine rules. Dead-code (`deslop/*`), the environment / supply-chain checks (`require-reduced-motion`, `require-pnpm-hardening`), and adopted third-party plugins (`eslint`, `unicorn`, `react-hooks-js`, …) have no recipe, so their links 404. The directive is now gated to engine rules, so agents are no longer sent to dead links.

- [#607](https://github.com/millionco/harness-doctor/pull/607) [`5dff3b5`](https://github.com/millionco/harness-doctor/commit/5dff3b5a5da033e0ae4cd5dd432a74d36ca7d143) Thanks [@rayhanadev](https://github.com/rayhanadev)! - Fix `harness-doctor --staged` (and other scans) hanging after the diagnostics summary is already printed. When an adopted lint config crashed oxlint on the first attempt, the oxlint runner's per-batch progress timer was left running while the scan silently retried with `extends` stripped — so the run finished and printed results, but the orphaned `setInterval` kept the Node event loop alive and the process never returned control to the shell. The batch loop now clears the timer in a `finally`, so it's always cleaned up even when a batch throws. See [#599](https://github.com/millionco/harness-doctor/issues/599).

- [#614](https://github.com/millionco/harness-doctor/pull/614) [`daef23c`](https://github.com/millionco/harness-doctor/commit/daef23ceff93634ffdb34c8e22e41610a07a596b) Thanks [@aidenybai](https://github.com/aidenybai)! - `jsx-key` no longer reports a missing key when a list element spreads the whole iteration item — `items.map((item) => <Item {...item} />)`. Spreading the row object is the canonical "this row carries its own identity" shape and was the dominant source of `jsx-key` noise on real lists, while rarely catching a genuine reorder bug. Genuine keyless lists still report: `items.map((item) => <Item name={item.name} />)`, index keys, array literals (`[<Item {...item} />]`), and spreads of anything other than the iteration variable.
...
- [#616](https://github.com/millionco/harness-doctor/pull/616) [`bb15252`](https://github.com/millionco/harness-doctor/commit/bb15252940bbd598846d7f7018df3fb86f11ea9f) Thanks [@rayhanadev](https://github.com/rayhanadev)! - Add an `--experimental-parallel [workers]` flag that runs the oxlint lint pass across multiple worker processes instead of one batch at a time. Harness Doctor's rules are oxlint JS plugins (single-threaded per process), so a serial scan only ever pins one core; `--experimental-parallel` fans the file batches out across the requested number of concurrent oxlint subprocesses, which scales the scan nearly linearly with CPU cores (measured ~3.5–4.6x on a 1,500-file project and ~4.6x on Sentry's 8,773 files) while producing byte-identical diagnostics.

  `--experimental-parallel` with no value auto-detects available cores; `--experimental-parallel <n>` caps the worker count; `HARNESS_DOCTOR_PARALLEL=<n>` seeds the default for flag-less / CI runs. The worker count is clamped to a safe range to bound peak memory, and the default remains serial so resource usage stays opt-in.

- [#601](https://github.com/millionco/harness-doctor/pull/601) [`5f7cc7c`](https://github.com/millionco/harness-doctor/commit/5f7cc7c36ed62b0c2264916f2aeb694e5713e821) Thanks [@rayhanadev](https://github.com/rayhanadev)! - Publish a JSON Schema for `harness-doctor.config.json` at `https://harness.doctor/schema/config.json`.

  Pointing `$schema` at the URL enables editor autocomplete, hover docs from the interface JSDoc, and typo warnings in any editor that understands JSON Schema. Closes [#497](https://github.com/millionco/harness-doctor/issues/497).
...
- [#617](https://github.com/millionco/harness-doctor/pull/617) [`9777f1a`](https://github.com/millionco/harness-doctor/commit/9777f1ac453d8211e90c66852e0527a0ec386bc6) Thanks [@rayhanadev](https://github.com/rayhanadev)! - Configure Harness Doctor with `doctor.config.{ts,js,mjs,cjs,mts,cts,json}` (or a `package.json#harnessDoctor` key), and add `harness-doctor rules` commands to list, explain, and configure rules without hand-editing config.
  ...
  - **`rules` commands.** `rules list` shows every rule and the severity it runs at; `rules explain <rule>` describes why a rule matters and how to tune it; and `rules set` / `enable` / `disable` / `category` / `ignore-tag` / `unignore-tag` edit your config for you. TS/JS configs are edited in place via `magicast` (formatting and comments preserved); JSON and `package.json` are edited as data; a `doctor.config.json` is created when no config exists. Rule references accept the full key (`harness-doctor/no-danger`), the bare id (`no-danger`), or a legacy key (`react/no-danger`).
  - **`doctor-explain` skill** (alias `doctor-config`), shipped via `harness-doctor install`, teaches coding agents to explain a rule before disabling it and to pick the narrowest control (rule severity vs category vs tag vs `surfaces`).

  **Breaking:** the config file is now `doctor.config.*` instead of `harness-doctor.config.json`. The next time you run `harness-doctor` interactively, an existing `harness-doctor.config.json` is automatically migrated to a typed `doctor.config.ts` (settings preserved, `$schema` dropped) and you're told once — CI, coding-agent, `--staged`, JSON/score, and non-TTY runs are left untouched (a warning still nudges them). The `package.json#harnessDoctor` key is unchanged.

- [#596](https://github.com/millionco/harness-doctor/pull/596) [`6e59f10`](https://github.com/millionco/harness-doctor/commit/6e59f10ef8b2173f0c98a653b13702d84f6471e7) Thanks [@aidenybai](https://github.com/aidenybai)! - Cleaner scan output and smarter file scoping:
...
- [#621](https://github.com/millionco/harness-doctor/pull/621) [`24425b1`](https://github.com/millionco/harness-doctor/commit/24425b113adcd965545b8c790af85c13a0d86c86) Thanks [@NisargIO](https://github.com/NisargIO)! - Add Sentry crash reporting to the CLI. Uncaught errors that reach the CLI's error funnels are now captured via `@sentry/node` and flushed before the process exits, each enriched with a `run` context snapshot (version, node/platform/arch, the invocation `command`/`argv`, `cwd`, CI provider, coding agent, interactivity, and JSON mode) to make crashes triage-able. Sentry initializes as the first statement of the CLI entry so its global handlers are armed before any command runs, and it's scoped to the CLI only — the programmatic `@harness-doctor/api` library never initializes Sentry.
...
- [#628](https://github.com/millionco/harness-doctor/pull/628) [`e9e71bb`](https://github.com/millionco/harness-doctor/commit/e9e71bbc2f98e7175136918b1a3de134e9d7cb87) Thanks [@rayhanadev](https://github.com/rayhanadev)! - Deepen the CLI's Sentry integration: uploaded source maps, unified tracing, and richer run context.
  ...
  - **Tracing / OpenTelemetry**: each run is now a Sentry transaction, and the existing Effect instrumentation (`runInspect` plus every `Effect.fn("Service.method")` span) is bridged straight into Sentry as one unified per-run trace. If a user has their own OTLP backend configured (`HARNESS_DOCTOR_OTLP_*`), that still wins and the Effect trace is additionally parented under the Sentry trace so the two share a `trace_id`. Tracing is tunable via `SENTRY_TRACES_SAMPLE_RATE` (set to `0` to disable; default samples every run).
  ...
  Reporting remains opt-out and CLI-only: `--no-score` disables Sentry entirely (crash reporting and tracing), it's skipped under test runs, and the programmatic `@harness-doctor/api` library never initializes Sentry.

- [#622](https://github.com/millionco/harness-doctor/pull/622) [`0938376`](https://github.com/millionco/harness-doctor/commit/0938376e9c60dcc52fdf4691c15db49feb603033) Thanks [@NisargIO](https://github.com/NisargIO)! - Show `"warning"`-severity diagnostics by default again. A scan that reports only errors is too generous a bar for a health check, so warnings surface on every surface (CLI, PR comment, score, `--fail-on`) out of the box. Opt out with `--no-warnings` or `"warnings": false`; per-rule / per-category severity overrides still win as before.
...
- [#586](https://github.com/millionco/harness-doctor/pull/586) [`9d20182`](https://github.com/millionco/harness-doctor/commit/9d20182ac14c7d89d52de98d2a8bfa9bad74f99e) Thanks [@aidenybai](https://github.com/aidenybai)! - Fix the CLI hanging after the post-scan prompts. Interactive prompts re-ref stdin via `readline` and never release it on close, undoing the startup `unrefStdin()` and holding the one-shot CLI's event loop open. The shared `prompts` wrapper now re-unrefs stdin once each prompt settles.

- [`d40a933`](https://github.com/millionco/harness-doctor/commit/d40a9339cfacd818b199be5b87bb68129d07b336) Thanks [@aidenybai](https://github.com/aidenybai)! - Trigger a release.
...
- [#570](https://github.com/millionco/harness-doctor/pull/570) [`d917f62`](https://github.com/millionco/harness-doctor/commit/d917f62ed6215e9a984c9bfa83940bba723ff5de) Thanks [@aidenybai](https://github.com/aidenybai)! - Add the `no-prop-types` architecture rule. React 19 removed runtime `propTypes` validation entirely — React no longer reads `Component.propTypes`, so invalid props that used to log a console warning now pass silently. The rule flags `Component.propTypes = { ... }` assignments and `static propTypes` class fields on component-cased identifiers, and is version-gated to React 19+ (`requires: ["react:19"]`) so projects where `propTypes` still runs stay quiet. It steers users toward TypeScript prop types plus explicit runtime validation. See [#460](https://github.com/millionco/harness-doctor/issues/460).

- [#582](https://github.com/millionco/harness-doctor/pull/582) [`b2934f9`](https://github.com/millionco/harness-doctor/commit/b2934f93e439027ed132e40688d45ef682f05efb) Thanks [@rayhanadev](https://github.com/rayhanadev)! - Fix a `rn-no-raw-text` false positive on fbtee translation tags. fbtee's `<fbt>` / `<fbs>` (and namespaced children like `<fbt:param>`) are compile-time translation tags that disappear at build time, so text inside `<Text><fbt>…</fbt></Text>` is really rendered inside `<Text>` and is safe on React Native. The rule now treats `fbt` / `fbs` as transparent wrappers when every ancestor up to a text-handling component is also transparent, while still reporting raw text when an `<fbt>` is used outside a `<Text>` boundary. See [#581](https://github.com/millionco/harness-doctor/issues/581).

- [#557](https://github.com/millionco/harness-doctor/pull/557) [`67848ae`](https://github.com/millionco/harness-doctor/commit/67848aec9cba76fc44ec7e36ac3a2b0717f5bd28) Thanks [@aidenybai](https://github.com/aidenybai)! - Scope React subproject discovery so running `harness-doctor` from a home directory no longer reports unrelated, vendored projects as ambiguous candidates. When the scan root has no `package.json` or workspace manifest, the filesystem crawl now skips OS/editor app-data directories (`AppData`, `Library`, …) and stops descending past a fixed depth. Previously a home-directory scan could surface React packages bundled inside editor installs (e.g. a VS Code extension under `AppData`) alongside real projects, aborting with `Multiple React projects found`. See [#545](https://github.com/millionco/harness-doctor/issues/545).

- [#576](https://github.com/millionco/harness-doctor/pull/576) [`e7a998a`](https://github.com/millionco/harness-doctor/commit/e7a998a9fa00a894177d9afc22e3a78b814392f1) Thanks [@aidenybai](https://github.com/aidenybai)! - Unref `process.stdin` at CLI startup so an inherited stdin pipe/socket can no longer keep the event loop alive after a scan completes. Previously `harness-doctor --json` (and other one-shot runs) could finish the scan and flush the full report yet never exit when launched by a parent that holds the stdin write-end open (eval runners, CI harnesses, editor integrations) — Node kept the loop alive on the idle `Socket fd=0`. Interactive prompts are unaffected because `prompts`' `readline` interface re-refs stdin on demand.
...
- Add Preact project detection so `harness-doctor inspect` recognizes Preact workspaces, including Vite + Preact projects that still report `vite` as their framework, and enables the bundled Preact rule family.
...
- **Agent install hint.** When running inside a coding agent, the CLI suggests `harness-doctor install` to set up the agent skill for in-editor diagnostics.
...
- **`/doctor` triage skill.** The Harness Doctor agent skill now includes a `/doctor` command that fetches the canonical playbook for full-project triage.
...
- **First-run onboarding.** New users see a brief walkthrough on their first `harness-doctor` invocation explaining what the tool does and how to read the report.
...
- **New `@harness-doctor/api` package.** Programmatic `diagnose()` entry point backed by the same `runInspect` orchestrator the CLI uses, with typed `HarnessDoctorError` failures and `Effect.catchReasons` dispatch.
...
- **Native agent hook installer.** `harness-doctor install` writes post-checkout / post-merge git hooks that auto-run the scan on relevant file changes.

- **Opt-in OpenTelemetry.** `HARNESS_DOCTOR_OTLP_ENDPOINT` + `HARNESS_DOCTOR_OTLP_AUTH_HEADER` ship every service span to an OTLP backend.
...
- Collapse `@harness-doctor/types` and `@harness-doctor/project-info` into `@harness-doctor/core`.
...
- Fix vite build configuration for bundling workspace dependencies so `npx harness-doctor` resolves internal workspace imports correctly.
...
- [`29b7229`](https://github.com/millionco/harness-doctor/commit/29b7229ea144cfe80c4401391eed3aa035071bcd) - Add `oxlint-plugin-harness-doctor` to `dependencies` so it is installed
  ...
  dependency list, causing `ERR_MODULE_NOT_FOUND` on `npx harness-doctor`.
...
- [`10d5de8`](https://github.com/millionco/harness-doctor/commit/10d5de804fe9c03fa9f18e5350bb26965a5108ac) - Fix workspace packages not being bundled into dist, causing
  `ERR_MODULE_NOT_FOUND: Cannot find package '@harness-doctor/core'`
...
- [#202](https://github.com/millionco/harness-doctor/pull/202) [`53fa4df`](https://github.com/millionco/harness-doctor/commit/53fa4dffe837e0157fb850fef700fccaaec191ea) - Detect the project's Tailwind version (`tailwindcss` in `package.json`,
...
- [#271](https://github.com/millionco/harness-doctor/pull/271) [`7a7ec84`](https://github.com/millionco/harness-doctor/commit/7a7ec84fad631d96f70279394be5f086b8424d17) - **Per-surface diagnostic controls (CLI).** New
  ...
  `@harness-doctor/core` surface filter into `cli/commands/inspect.ts`
...
- Inherits the rule-fix wave from
  `oxlint-plugin-harness-doctor@0.2.0-beta.5` (rules are bundled into
  the CLI):
  `no-secrets-in-client-code` scoping
  ([#252](https://github.com/millionco/harness-doctor/pull/252)),
  `nextjs-no-side-effect-in-get-handler` safe local bindings
  ([#260](https://github.com/millionco/harness-doctor/pull/260)),
  `async-defer-await` false-positive fixes
  ([#265](https://github.com/millionco/harness-doctor/pull/265)),
  `js-length-check-first` `&&`-chain detection
  ([#269](https://github.com/millionco/harness-doctor/pull/269)),
  `async-parallel` test / browser-fixture suppression
  ([#270](https://github.com/millionco/harness-doctor/pull/270)),
  `js-combine-iterations` lazy `Iterator` skip
  ([#272](https://github.com/millionco/harness-doctor/pull/272)), and
  `no-prevent-default` framework awareness
  ([#274](https://github.com/millionco/harness-doctor/pull/274)).
...
- Add `oxlint-plugin-harness-doctor` to `dependencies` so it is installed
  alongside the CLI. The bundler correctly externalises the plugin (oxlint
  loads it by file path at runtime) but it was missing from the published
  dependency list, causing `ERR_MODULE_NOT_FOUND` on `npx harness-doctor`.
- Updated dependencies []:
...
- [`10d5de8`](https://github.com/millionco/harness-doctor/commit/10d5de804fe9c03fa9f18e5350bb26965a5108ac) - Fix workspace packages
  (`@harness-doctor/core`, `@harness-doctor/project-info`,
  `@harness-doctor/types`) not being bundled into the published `dist/`
  output, which caused
  `ERR_MODULE_NOT_FOUND: Cannot find package '@harness-doctor/core'`
  on `npx harness-doctor` after the package extraction in beta.2.
  Vite config now treats the workspace dependencies as bundle-time
  inputs.

- Inherits the
  [#253](https://github.com/millionco/harness-doctor/pull/253) `no-barrel-import`
  index-resolution fix from
  `oxlint-plugin-harness-doctor@0.2.0-beta.3` (rules are bundled into
  the CLI).
...
- [#249](https://github.com/millionco/harness-doctor/pull/249) [`f0198e2`](https://github.com/millionco/harness-doctor/commit/f0198e2f2d9560a15bdb4a78f4a378ca2ac5fcdd) - **Internal-package extraction.** The CLI no longer vendors project
  detection, the oxlint runner, scoring, or the shared type layer
  inline - those modules now live in
  `@harness-doctor/types`, `@harness-doctor/project-info`, and
  `@harness-doctor/core` and are consumed as workspace dependencies.
  Bundled into `dist/` on publish (see also
  [#253](https://github.com/millionco/harness-doctor/pull/253) bundler
  follow-up in beta.3). The `harness-doctor`, `harness-doctor inspect`,
  and `harness-doctor install` binaries are surface-compatible with
  0.1.6.

- [#250](https://github.com/millionco/harness-doctor/pull/250) [`6e2ee9d`](https://github.com/millionco/harness-doctor/commit/6e2ee9d474fddbde4c1246ff65b2f3e5bb3a42fc) - **CLI reorganised.** `src/cli/` is now split into `commands/` +
...
- [#208](https://github.com/millionco/harness-doctor/pull/208) [`8556b31`](https://github.com/millionco/harness-doctor/commit/8556b31d8e4e165f791db0aa60a6b038b18ec777) - **User-feedback sweep.** Reduce false positives across the design /
  Tailwind / state-and-effects rule groups, surface per-rule scoring
  contributions in `harness-doctor inspect`, and add `--severity` /
  `--rule-set` CLI options plus their `harness-doctor.config.json`
  counterparts. Closes the bulk of the feedback collected on 0.1.x.

- [#174](https://github.com/millionco/harness-doctor/pull/174) - Forward
  `reactMajorVersion` through the programmatic `diagnose()` entry
  point so embedders running harness-doctor inside their own pipeline
...
- [`e9e4217`](https://github.com/millionco/harness-doctor/commit/e9e4217711341cf2bfb7c1b11f37caaae44df7c3) - Harden `discover-project` and `resolve-diagnose-target`: tighter
...
- [`b06b768`](https://github.com/millionco/harness-doctor/commit/b06b768) ([#193](https://github.com/millionco/harness-doctor/pull/193)) - `diagnose()` now falls back to the first nested React subproject when the
...
- [#200](https://github.com/millionco/harness-doctor/pull/200) - Typed
...
- [#201](https://github.com/millionco/harness-doctor/pull/201) - Integrate
  ...
  alongside harness-doctor's own state-and-effects rules.

- [#194](https://github.com/millionco/harness-doctor/pull/194) - Resolve
...
- [#196](https://github.com/millionco/harness-doctor/pull/196) - Match
  `harness-doctor-disable*` suppression comments that carry descriptive
  trailing text (e.g. `// harness-doctor-disable-next-line rule -- why`)
  instead of requiring a bare comment. Resolves
  [#159](https://github.com/millionco/harness-doctor/issues/159).

- [#198](https://github.com/millionco/harness-doctor/pull/198) - Expose
  `--why` as a documented public alias for `--explain` in the CLI.
  Resolves [#161](https://github.com/millionco/harness-doctor/issues/161).

- [#195](https://github.com/millionco/harness-doctor/pull/195) - The
  ...
  Resolves [#190](https://github.com/millionco/harness-doctor/issues/190).

- [#197](https://github.com/millionco/harness-doctor/pull/197) - Docs:
  clarify that `ignore.overrides` covers per-file rule ignores.

- [#199](https://github.com/millionco/harness-doctor/pull/199) - Docs:
...
- [`a63d5d5`](https://github.com/millionco/harness-doctor/commit/a63d5d5d0bbff26e7367a0b6d634aeb089a935ef) - CLI scan output reformat. Adds `utils/wrap-indented-text.ts` for
...
- [#184](https://github.com/millionco/harness-doctor/pull/184) - Add a
  ...
  [#183](https://github.com/millionco/harness-doctor/issues/183).

- [#182](https://github.com/millionco/harness-doctor/pull/182) - Restore
...
- [#181](https://github.com/millionco/harness-doctor/pull/181) - Website
  fix: keep the diagnostic count next to the rule name on narrow widths
  in the leaderboard / diagnostic listings.

- [`cca5808`](https://github.com/millionco/harness-doctor/commit/cca5808) - Promote `react-hooks-js/*` diagnostics to errors so projects with
  React Hooks rule violations no longer pass with a clean score.

- [`9ee3a6d`](https://github.com/millionco/harness-doctor/commit/9ee3a6d) - Refresh the website's terminal demo to match the new CLI output
...
- [`6ddb02c`](https://github.com/millionco/harness-doctor/commit/6ddb02c6b08fbfb06e2429d3cabd338c91891cd6) - Polish follow-up to the 0.1.1 CLI redesign. Consolidates duplicated
...
- [#178](https://github.com/millionco/harness-doctor/pull/178) - CLI
...
- d71a6bf: feat(harness-doctor): ship rules as an ESLint plugin (`harness-doctor/eslint-plugin`)

  The same Harness Doctor rule set that powers the CLI scan and the
  `harness-doctor/oxlint-plugin` export is now available as a first-class
  ...
  hook already speaks ESLint - no separate `harness-doctor` invocation
  needed.

  ```js
  // eslint.config.js
  import harnessDoctor from "harness-doctor/eslint-plugin";

  export default [
    harnessDoctor.configs.recommended,
    harnessDoctor.configs.next, // composable framework presets
    harnessDoctor.configs["react-native"],
    harnessDoctor.configs["tanstack-start"],
    harnessDoctor.configs["tanstack-query"],
    // harnessDoctor.configs.all, // every rule at harness-doctor's default severity
  ];
  ```
  ...
  harness-doctor CLI emits to oxlint, so behavior stays in lock-step
  between engines. You can also cherry-pick individual rules under the
  `harness-doctor/*` namespace.
  ...
  [#143](https://github.com/millionco/harness-doctor/issues/143).

- d71a6bf: feat(harness-doctor): adopt the project's existing oxlint / eslint config and factor those rules into the score
  ...
  harness-doctor now folds that config into the same scan via oxlint's
  `extends` field. The user's existing rules fire alongside the curated
  harness-doctor rule set, and the resulting diagnostics count toward the
  ...
  **Behavior change on upgrade.** Projects with an existing
  ...
  `"adoptExistingLintConfig": false` in `harness-doctor.config.json` (or the
  `"harnessDoctor"` key in `package.json`) to preserve the previous
  behavior. `customRulesOnly: true` also implies opt-out, since that mode
  runs only the `harness-doctor/*` plugin.

  **Resilience.** If oxlint can't load the user's config (broken JSON,
  missing plugin, unknown rule name), harness-doctor logs the reason on
  ...
  **Coverage broadened.** Diagnostics on `.ts` and `.js` files are now
  ...
  / `.jsx`). This affects harness-doctor's own JS-performance / bundle-size
  ...
  **Limitations.** Only JSON configs are picked up: oxlint's `extends`
  ...
  category-level enables (`"categories": {...}`) do not - harness-doctor's
  local categories block always wins. Closes #143.

- d71a6bf: feat(harness-doctor): add 11 new lint rules - 3 state / correctness, 8 design system

  **3 new state / correctness rules** (all `warn`):

  - `harness-doctor/no-direct-state-mutation` - flags `state.foo = x` and
    ...
    false-positive.
  - `harness-doctor/no-set-state-in-render` - flags only **unconditional**
    top-level setter calls so the canonical
    `if (prev !== prop) setPrev(prop)` derive-from-props pattern stays
    clean.
  - `harness-doctor/no-uncontrolled-input` - catches `<input value={…}>`
  ...
  **8 new design-system rules in `react-ui.ts`** (all `warn`):
  ...
  - `harness-doctor/design-no-bold-heading` -
    `font-bold` / `font-extrabold` / `font-black` or inline
    `fontWeight ≥ 700` on `h1`–`h6`.
  - `harness-doctor/design-no-redundant-padding-axes` - collapse
    `px-N py-N` → `p-N`.
  - `harness-doctor/design-no-redundant-size-axes` - collapse `w-N h-N` →
    `size-N`.
  - `harness-doctor/design-no-space-on-flex-children` - use `gap-*` over
    `space-*-*`.
  - `harness-doctor/design-no-em-dash-in-jsx-text` - em dashes in JSX
    text.
  - `harness-doctor/design-no-three-period-ellipsis` - `Loading...` →
    `Loading…`.
  - `harness-doctor/design-no-default-tailwind-palette` -
    `indigo-*` / `gray-*` / `slate-*` reads as the Tailwind template
    default; reports every offending token in the className (not just
    the first).
  - `harness-doctor/design-no-vague-button-label` - `OK` / `Continue` /
...
- d71a6bf: remove(harness-doctor): drop browser entrypoints, browser CLI, and the
  `harness-doctor-browser` workspace package

  **Removed package exports.** `harness-doctor/browser` and
  `harness-doctor/worker` are no longer published. Imports of either subpath
  ...
  `harness-doctor@0.0.47` or vendor the relevant modules from the
  `archive/browser` git branch.

  **Removed CLI subcommand.** `harness-doctor browser …` (`start`, `stop`,
  ...
  **Removed companion package.** The `harness-doctor-browser` npm package
  ...
  **Why.** The browser surface area was unused inside the monorepo (the
  ...
  consumers. Removing it tightens what `harness-doctor` is responsible for:
  the diagnostics CLI, the Node `harness-doctor/api`, and the
  `harness-doctor/eslint-plugin` / `harness-doctor/oxlint-plugin` exports.
...
- 2aebfa6: fix(harness-doctor): support block comment forms of `harness-doctor-disable-line` / `harness-doctor-disable-next-line`

  The inline-suppression matcher previously only recognized line comments
  (`// harness-doctor-disable-…`). Block comments - including the JSX form
  `{/* harness-doctor-disable-next-line … */}`, which is the only suppression
  form legal directly inside JSX - were silently ignored, forcing users to
  write `{/* // harness-doctor-disable-line … */}` as a workaround. Both forms
...
- 2aebfa6: fix(harness-doctor): stop flagging `useState` as `useRef` when state reaches render through `useMemo`, derived values, or context `value`
...
- [#148](https://github.com/millionco/harness-doctor/pull/148) ([`3f5c031`](https://github.com/millionco/harness-doctor/commit/3f5c031)) - Add the (now-removed in 0.1.0) `harness-doctor browser` CLI subcommand
...
- [#152](https://github.com/millionco/harness-doctor/pull/152) ([`8f10098`](https://github.com/millionco/harness-doctor/commit/8f10098)) - 4 new React 18→19 migration rules: pre-flight checks for the
  `forwardRef`-deprecation / new context API / new ref-callback
  cleanup / `use()` adoption migration paths.

- [#154](https://github.com/millionco/harness-doctor/pull/154) ([`276ea2f`](https://github.com/millionco/harness-doctor/commit/276ea2f)) - Add `prefer-use-sync-external-store` rule. Catches `useEffect`-based
  subscriptions that should be `useSyncExternalStore` (concurrent-mode
  safe, tearing-resistant).

- [#155](https://github.com/millionco/harness-doctor/pull/155) ([`2240b1f`](https://github.com/millionco/harness-doctor/commit/2240b1f)) - Add `no-event-trigger-state` rule. Flags state that is only ever
  written from an event handler and only read from the rendered JSX
  return - a frequent prop-derivation antipattern.

- [#156](https://github.com/millionco/harness-doctor/pull/156) ([`4b92f50`](https://github.com/millionco/harness-doctor/commit/4b92f50)) - Add `no-effect-chain` rule. Flags `useEffect` chains where one
...
- [#157](https://github.com/millionco/harness-doctor/pull/157) ([`0be99ad`](https://github.com/millionco/harness-doctor/commit/0be99ad)) - Comprehensive useEffect analyzer: three new rules
  (`no-mutable-in-deps`, `no-mirror-prop-effect`, `effect-needs-cleanup`)
  plus shared dependency-tracking infrastructure.

- [#162](https://github.com/millionco/harness-doctor/pull/162) ([`945138d`](https://github.com/millionco/harness-doctor/commit/945138d)) - Gate `prefer-use-effect-event` behind React 19+ so the suggestion
  doesn't fire on React 18 projects (where `useEffectEvent` is not
  available).

- [#163](https://github.com/millionco/harness-doctor/pull/163) ([`c20857e`](https://github.com/millionco/harness-doctor/commit/c20857e)) - `no-effect-event-handler` honors empty-frame barriers in prop-stack
  lookups so callbacks hoisted out of effects don't inherit the
  surrounding effect classification.

- [#165](https://github.com/millionco/harness-doctor/pull/165) ([`78db3b2`](https://github.com/millionco/harness-doctor/commit/78db3b2)) - Multi-line JSX and stacked suppression comments, per-file rule
  overrides, and near-miss hints for misspelled rule ids.

- [#166](https://github.com/millionco/harness-doctor/pull/166) ([`8745c34`](https://github.com/millionco/harness-doctor/commit/8745c34)) - Suppression follow-ups: audit-mode handling, overrides-config
...
- [#167](https://github.com/millionco/harness-doctor/pull/167) ([`d3e26d6`](https://github.com/millionco/harness-doctor/commit/d3e26d6)) - Refactor: consolidate state-and-effects rule plumbing.

- [#169](https://github.com/millionco/harness-doctor/pull/169) ([`50d08fd`](https://github.com/millionco/harness-doctor/commit/50d08fd)) - AGENTS.md compliance pass on the state-and-effects rule directory.

- [#170](https://github.com/millionco/harness-doctor/pull/170) ([`97cb1bb`](https://github.com/millionco/harness-doctor/commit/97cb1bb)) - Collapse non-verbose CLI diagnostics to the top 3 rules so the
  default scan output stays scannable on large projects; `--verbose`
  restores the full listing.

- [#172](https://github.com/millionco/harness-doctor/pull/172) ([`2a1b0ae`](https://github.com/millionco/harness-doctor/commit/2a1b0ae)) - Tighten state-and-effects rules against false positives across the
  rules-of-hooks / handler-detection / render-reachable code paths.

- [#174](https://github.com/millionco/harness-doctor/pull/174) ([`4fb4d27`](https://github.com/millionco/harness-doctor/commit/4fb4d27)) - Forward `reactMajorVersion` through the programmatic `diagnose()`
  entry point so embedders get the same React-19 rule gating the CLI
  uses.

- [#177](https://github.com/millionco/harness-doctor/pull/177) ([`01c38a7`](https://github.com/millionco/harness-doctor/commit/01c38a7)) - Harden rules against prototype-pollution false positives and quiet
...
- 6a0e6d6: chore(harness-doctor): bump oxlint to ^1.62.0
...
- dbf200d: fix(harness-doctor): filter React Compiler rules to those the loaded `eslint-plugin-react-hooks` actually exports
...
- c13a8df: fix(harness-doctor): skip React Compiler rules when `eslint-plugin-react-hooks` isn't installed
...
- 6b07924: `harness-doctor install` now delegates skill installation to
...
- [`57467cd`](https://github.com/millionco/harness-doctor/commit/57467cd) - Patch follow-up to the 0.0.43 ignore-respecting refactor: misc
...
- **Respect existing eslint / oxlint / prettier ignores by default.** Harness Doctor now honors `.gitignore`, `.eslintignore`, `.oxlintignore`, `.prettierignore`, and `.gitattributes` `linguist-vendored` / `linguist-generated` annotations, plus inline `// eslint-disable*` and `// oxlint-disable*` comments. Previously inline disable comments were neutralized so harness-doctor saw through every prior suppression - this surprised users who had `eslint-disable` in place for legitimate reasons. **Behavior change:** existing users may see fewer findings (previously-suppressed code is now correctly suppressed). To restore the old "audit everything" behavior, set `"respectInlineDisables": false` in `harness-doctor.config.json` or pass `--no-respect-inline-disables` on the CLI.
...
- [`1fdc9a0`](https://github.com/millionco/harness-doctor/commit/1fdc9a0) - Patch follow-up to the 0.0.39 browser-entrypoint work: misc
  bundling fixes for the now-removed `harness-doctor/browser` and
  `harness-doctor/worker` subpath exports.

## 0.0.40

### Patch Changes

- [`874f7bc`](https://github.com/millionco/harness-doctor/commit/874f7bc) - Publishing-pipeline retry of 0.0.39 (no code delta).

## 0.0.39

### Patch Changes

- [#134](https://github.com/millionco/harness-doctor/pull/134) ([`7da4ce4`](https://github.com/millionco/harness-doctor/commit/7da4ce4)) - Fix `TypeError: issues.files is not iterable` crash during dead-code
...
- [`5238e7b`](https://github.com/millionco/harness-doctor/commit/5238e7b) / [`061a794`](https://github.com/millionco/harness-doctor/commit/061a794) - Add the `harness-doctor/browser` entrypoint and a `diagnoseBrowser` /
...
- [`b5519b6`](https://github.com/millionco/harness-doctor/commit/b5519b6) - Inject the browser scorer at build time so the bundled
  `harness-doctor/browser` output omits the proxy-fetch / Node-only score
  path.
...
- [`8b0485a`](https://github.com/millionco/harness-doctor/commit/8b0485a) - GitHub Action improvements (input validation + step output cleanups),
  clickable file paths in CLI diagnostic output (terminal hyperlinks
  via OSC-8), and a website hydration fix.

- [`bb5188f`](https://github.com/millionco/harness-doctor/commit/bb5188f) - Document every config option supported by `harness-doctor.config.json`
  in the README.

- [`100731c`](https://github.com/millionco/harness-doctor/commit/100731c) - `install-skill` and `detect-agents` formatting fixes so CI's
...
- [`f1bd776`](https://github.com/millionco/harness-doctor/commit/f1bd776) - Republish 0.0.36 after a botched skill payload - the
...
- [#131](https://github.com/millionco/harness-doctor/pull/131) ([`ff5b637`](https://github.com/millionco/harness-doctor/commit/ff5b637)) - Frontend design-quality rules (initial cut of the design-system
  rule family - bold headings, redundant padding/size axes, vague
  button labels, etc.). Expanded substantially in 0.1.0.

- [`074f854`](https://github.com/millionco/harness-doctor/commit/074f854) - Add the `harness-doctor install` subcommand that installs the
  Harness Doctor SKILL.md into the user's configured coding agents.

## 0.0.35

### Patch Changes

- [`7136aa5`](https://github.com/millionco/harness-doctor/commit/7136aa5) - Republish 0.0.34 after a packaging hiccup; no code delta.

## 0.0.34

### Patch Changes

- [#124](https://github.com/millionco/harness-doctor/pull/124) ([`0eb635a`](https://github.com/millionco/harness-doctor/commit/0eb635a)) - Add TanStack Start rule family (router conventions, server-function
  hygiene, `useServerFn` adoption hints).

- [#129](https://github.com/millionco/harness-doctor/pull/129) ([`47105e9`](https://github.com/millionco/harness-doctor/commit/47105e9)) - Fix false positives for Next.js redirect guidance and React Compiler
  detection.
...
- [`87d4b86`](https://github.com/millionco/harness-doctor/commit/87d4b86) - Republish 0.0.32 after a packaging hiccup; no code delta.

## 0.0.32

### Patch Changes

- [#120](https://github.com/millionco/harness-doctor/pull/120) ([`10bd788`](https://github.com/millionco/harness-doctor/commit/10bd788)) - Address multiple GitHub issues (#117, #113, #119, #106): assorted
  rule false-positive fixes, CLI option polish, and detection
  hardening.

- [#122](https://github.com/millionco/harness-doctor/pull/122) ([`901fce5`](https://github.com/millionco/harness-doctor/commit/901fce5)) - Restrict setter detection to direct `Identifier` callees so dynamic
...
- [#108](https://github.com/millionco/harness-doctor/pull/108) ([`1aef45a`](https://github.com/millionco/harness-doctor/commit/1aef45a)) - Resolve 7 GitHub issues across catalog resolution, file-ignore
  semantics, CLI ergonomics, React Native detection, Next.js
  detection, the `--offline` flag, and monorepo discovery.

- [#110](https://github.com/millionco/harness-doctor/pull/110) ([`720f421`](https://github.com/millionco/harness-doctor/commit/720f421)) - Apply `ignore.files` as a pre-filter so ignored files are skipped
...
- [`c405f4a`](https://github.com/millionco/harness-doctor/commit/c405f4a) - Resolve multiple GitHub issues (#71, #72, #76, #77, #83, #84, #86,
  #87, #89, #92, #93, #94): broad rule false-positive sweep across
  detection, scoring, and rule output formatting.

- [`97b21f1`](https://github.com/millionco/harness-doctor/commit/97b21f1) - Replace `fs.existsSync` with the shared `isFile` utility for
...
- [#63](https://github.com/millionco/harness-doctor/pull/63) ([`9f51b9d`](https://github.com/millionco/harness-doctor/commit/9f51b9d)) - GitHub Action: add a `diff` input so the action can scan only files
  changed in the PR, plus optional PR-comment posting from the action
  step.

- [#64](https://github.com/millionco/harness-doctor/pull/64) ([`79af7ca`](https://github.com/millionco/harness-doctor/commit/79af7ca)) - Detect Expo and React Native and enable the RN-specific rule
  family when a project targets them.

- [#68](https://github.com/millionco/harness-doctor/pull/68) ([`1947859`](https://github.com/millionco/harness-doctor/commit/1947859)) - Rule / detection false-positive fixes (issue rollup).

## 0.0.28

### Patch Changes

- [`bd949cc`](https://github.com/millionco/harness-doctor/commit/bd949cc) - Bump the Node version requirement, enhance the linting process with
...
- [`370ea4c`](https://github.com/millionco/harness-doctor/commit/370ea4c) - Refactor CLI option handling and improve automated-environment
  detection (CI / agent contexts).

- [`051a02c`](https://github.com/millionco/harness-doctor/commit/051a02c) - Score-calculation refactor: extract shared constants and add the
  proxy-fetch fallback used by the in-browser scorer.

- [`bf21a87`](https://github.com/millionco/harness-doctor/commit/bf21a87) - Fix `--fix` deeplink rendering issues introduced when the install
...
- [`7716d6c`](https://github.com/millionco/harness-doctor/commit/7716d6c) - Integrate the SKILL.md installation prompt directly into the CLI
...
- [#44](https://github.com/millionco/harness-doctor/pull/44) ([`dcf4276`](https://github.com/millionco/harness-doctor/commit/dcf4276)) - Remove the silent global install introduced in 0.0.9; the CLI no
  longer auto-installs itself globally on first run. Resolves
  [#43](https://github.com/millionco/harness-doctor/issues/43).

- [`7e20da1`](https://github.com/millionco/harness-doctor/commit/7e20da1) - Refactor the diagnostic payload structure used by the
  score-estimation API and tighten its validation.

- [`da83168`](https://github.com/millionco/harness-doctor/commit/da83168) - Enhance the Harness Doctor skill installation script with detailed
...
- [#33](https://github.com/millionco/harness-doctor/pull/33) ([`c2b687a`](https://github.com/millionco/harness-doctor/commit/c2b687a)) - Detect React Compiler in Expo projects.

- [#37](https://github.com/millionco/harness-doctor/pull/37) ([`7f49833`](https://github.com/millionco/harness-doctor/commit/7f49833)) - Chore: add node.js to leaderboard.

## 0.0.23

### Patch Changes

- [`c05f4b0`](https://github.com/millionco/harness-doctor/commit/c05f4b0) / [`b33976c`](https://github.com/millionco/harness-doctor/commit/b33976c) - Patch sweep - assorted rule and CLI false-positive fixes.

## 0.0.22

### Patch Changes

- [`84bb6d5`](https://github.com/millionco/harness-doctor/commit/84bb6d5) / [`61406e0`](https://github.com/millionco/harness-doctor/commit/61406e0) / [`1b07fa2`](https://github.com/millionco/harness-doctor/commit/1b07fa2) / [`0299fc4`](https://github.com/millionco/harness-doctor/commit/0299fc4) - Patch sweep - rule false-positive fixes and formatting follow-ups.

## 0.0.21

### Patch Changes

- [`73da9e2`](https://github.com/millionco/harness-doctor/commit/73da9e2) - Add the `--offline` flag so the CLI skips network calls (telemetry,
...
- [`45a3d33`](https://github.com/millionco/harness-doctor/commit/45a3d33) / [`d90a60d`](https://github.com/millionco/harness-doctor/commit/d90a60d) / [`b2d0b64`](https://github.com/millionco/harness-doctor/commit/b2d0b64) - Surface stderr from `oxlint` / `knip` invocations so failures
...
- [`b8cc03a`](https://github.com/millionco/harness-doctor/commit/b8cc03a) / [`2d2f7a1`](https://github.com/millionco/harness-doctor/commit/2d2f7a1) - Update CLI options, enhance configuration documentation, and
  miscellaneous fixes.
...
- [#17](https://github.com/millionco/harness-doctor/pull/17) ([`64d837b`](https://github.com/millionco/harness-doctor/commit/64d837b)) - Add `typescript` as a direct dependency to resolve the knip peer
...
- [`5c2119b`](https://github.com/millionco/harness-doctor/commit/5c2119b) / [`116d6cd`](https://github.com/millionco/harness-doctor/commit/116d6cd) - Enhance error handling in dead-code detection so a knip failure no
...
- [`06fb14e`](https://github.com/millionco/harness-doctor/commit/06fb14e) - Improve error handling in the linting and dead-code analysis paths
  so failures log a useful message instead of a stack trace.

- [`595ca55`](https://github.com/millionco/harness-doctor/commit/595ca55) - Remove the video package from `main` (kept on a separate branch for
  asset generation).
...
- [`e8cfef0`](https://github.com/millionco/harness-doctor/commit/e8cfef0) / [`1410650`](https://github.com/millionco/harness-doctor/commit/1410650) - Update `harness-doctor` package configuration / constants and refresh
...
- [`90ffa0a`](https://github.com/millionco/harness-doctor/commit/90ffa0a) - Add `llms.txt` so models discovering the package via the npm
  registry can find structured docs.

- [`e218d63`](https://github.com/millionco/harness-doctor/commit/e218d63) - Format the leaderboard data files to satisfy CI `format:check`.

## 0.0.13

### Patch Changes

- [#11](https://github.com/millionco/harness-doctor/pull/11) ([`2d2f779`](https://github.com/millionco/harness-doctor/commit/2d2f779)) - Harness Doctor branding refresh: theme-aware README logo + asset
  refresh. Reverted in [`28d820a`](https://github.com/millionco/harness-doctor/commit/28d820a) when the assets failed to render
  correctly on npm.

- [#12](https://github.com/millionco/harness-doctor/pull/12) ([`5fc130d`](https://github.com/millionco/harness-doctor/commit/5fc130d)) - Fix the favicon badge ring cutout.

- [#14](https://github.com/millionco/harness-doctor/pull/14) ([`67bad8f`](https://github.com/millionco/harness-doctor/commit/67bad8f)) - Set the Twitter image to the brand banner.

## 0.0.12

### Patch Changes

- [`b200689`](https://github.com/millionco/harness-doctor/commit/b200689) / [`4519747`](https://github.com/millionco/harness-doctor/commit/4519747) / [`7f0b4d2`](https://github.com/millionco/harness-doctor/commit/7f0b4d2) / [`6d1ae5e`](https://github.com/millionco/harness-doctor/commit/6d1ae5e) / [`5688c1f`](https://github.com/millionco/harness-doctor/commit/5688c1f) / [`6dd481a`](https://github.com/millionco/harness-doctor/commit/6dd481a) / [`ce8437e`](https://github.com/millionco/harness-doctor/commit/ce8437e) - Iteration sweep on detection / output formatting / README copy
...
- [`db319d0`](https://github.com/millionco/harness-doctor/commit/db319d0) / [`358750a`](https://github.com/millionco/harness-doctor/commit/358750a) - Environment-variable handling fix: thread the documented
  `HARNESS_DOCTOR_*` env vars through the score-estimation API call so
...
- [`e5ef934`](https://github.com/millionco/harness-doctor/commit/e5ef934) - "Almost ready" milestone - the rule pipeline + scoring + CLI surface
...
- [#2](https://github.com/millionco/harness-doctor/pull/2) ([`1ae6094`](https://github.com/millionco/harness-doctor/commit/1ae6094)) - Improve the `--prompt` clipboard output emitted for agent fixes
  (Ami-style copy block formatting).

- [#3](https://github.com/millionco/harness-doctor/pull/3) ([`f664ca2`](https://github.com/millionco/harness-doctor/commit/f664ca2)) - Fix the multiselect `a`-key select-all behavior in the interactive
  CLI prompt.

- [#4](https://github.com/millionco/harness-doctor/pull/4) ([`940e82a`](https://github.com/millionco/harness-doctor/commit/940e82a)) - Indent multi-line diagnostic help output in the CLI scan summary.

- [#5](https://github.com/millionco/harness-doctor/pull/5) ([`00f21ec`](https://github.com/millionco/harness-doctor/commit/00f21ec)) - Color diagnostic counts by severity in the CLI summary.

- [#6](https://github.com/millionco/harness-doctor/pull/6) ([`c55edc9`](https://github.com/millionco/harness-doctor/commit/c55edc9)) - Add severity icons to the summary counts.

- [#7](https://github.com/millionco/harness-doctor/pull/7) ([`8894a54`](https://github.com/millionco/harness-doctor/commit/8894a54)) - Frame the summary footer output (boxed border around the final
  score line).

- [#9](https://github.com/millionco/harness-doctor/pull/9) ([`a66c977`](https://github.com/millionco/harness-doctor/commit/a66c977)) - Move the website video overlays to a bottom-gradient layout and
  darken them.

- [`b5ea69b`](https://github.com/millionco/harness-doctor/commit/b5ea69b) - Add the GitHub Action for CI integration and fix monorepo scanning
  inside CI environments.

- [`578e75a`](https://github.com/millionco/harness-doctor/commit/578e75a) - Auto-install globally in the background when run via `npx`. (Later
  removed in 0.0.25 / #44 because it surprised users.)

- [`bde1167`](https://github.com/millionco/harness-doctor/commit/bde1167) - Add the video package and Ami skills for marketing-asset
  generation.
...
- [`19fa34b`](https://github.com/millionco/harness-doctor/commit/19fa34b) - Resolve a merge conflict in `cli.ts` introduced in 0.0.7.

- [`eef87e0`](https://github.com/millionco/harness-doctor/commit/eef87e0) - Use a single deeplink for `--fix` instead of the two-step deeplink

  - sleep dance.

- [`9cf691d`](https://github.com/millionco/harness-doctor/commit/9cf691d) / [`21c0d61`](https://github.com/millionco/harness-doctor/commit/21c0d61) - Skill install prompt copy refresh and README cleanup (remove the
...
- [`2ae9b87`](https://github.com/millionco/harness-doctor/commit/2ae9b87) - Fix the `--fix` deeplink to open the project with the correct cwd
...
- [`f9157c7`](https://github.com/millionco/harness-doctor/commit/f9157c7) - Add the `no-side-effect-in-get-handler` rule and export the oxlint
  plugin as a standalone entrypoint.

- [`9965871`](https://github.com/millionco/harness-doctor/commit/9965871) / [`80d676e`](https://github.com/millionco/harness-doctor/commit/80d676e) - Add the website with the animated CLI terminal landing page, and
  update the README with consumer-friendly docs.

- [`8379064`](https://github.com/millionco/harness-doctor/commit/8379064) / [`de1cbd4`](https://github.com/millionco/harness-doctor/commit/de1cbd4) - Add `fix` and `install-ami` commands that deeplink into Ami for
  automated rule application.

- [`330afc2`](https://github.com/millionco/harness-doctor/commit/330afc2) / [`7aa7b3f`](https://github.com/millionco/harness-doctor/commit/7aa7b3f) / [`05d2f79`](https://github.com/millionco/harness-doctor/commit/05d2f79) / [`4dfaeab`](https://github.com/millionco/harness-doctor/commit/4dfaeab) - Add ASCII doctor face / box branding to the CLI score output and
  the website terminal.

- [`b1f1abc`](https://github.com/millionco/harness-doctor/commit/b1f1abc) - Add the CI workflow for e2e tests, lint, and format.

- [`4b481c8`](https://github.com/millionco/harness-doctor/commit/4b481c8) - Add the Harness Doctor skill and the install prompt on first run.

## 0.0.5

### Patch Changes

- [`ccf404a`](https://github.com/millionco/harness-doctor/commit/ccf404a) - Gracefully handle failures in `oxlint`, the reduced-motion check,
  and summary-file writing so a single subsystem can't take down the
  scan.

- [`f1407d7`](https://github.com/millionco/harness-doctor/commit/f1407d7) - Gracefully handle knip failures on non-React config files.
...
- [`2d9a69b`](https://github.com/millionco/harness-doctor/commit/2d9a69b) - Add actionable help text, animation rules, per-rule summary files,
  and strengthen the framework / dependency detection.

- [`327c076`](https://github.com/millionco/harness-doctor/commit/327c076) - Move the website source into `packages/website`.

## 0.0.3

### Patch Changes

- [`680e7c4`](https://github.com/millionco/harness-doctor/commit/680e7c4) - Reduce default scan noisiness - tighter default rule severity
...
- [`a8770b7`](https://github.com/millionco/harness-doctor/commit/a8770b7) - Add CLI scaffolding with the initial oxlint integration: scan
...
- [`f50426b`](https://github.com/millionco/harness-doctor/commit/f50426b) - Initial publish - empty package scaffold to claim the npm name.
