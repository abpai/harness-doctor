...
import { resolveScanTarget } from "@harness-doctor/core";
import type { InspectResult } from "@harness-doctor/core";
...
vi.mock("@harness-doctor/core", async (importOriginal) => {
  const actual = await importOriginal<typeof import("@harness-doctor/core")>();
  return {
    ...actual,
...
describe("inspectAction setup prompt", () => {
  ...
  it("resolves setup from selected projects, not only completed diff scans", async () => {
    const rootDirectory = fs.mkdtempSync(path.join(os.tmpdir(), "harness-doctor-inspect-action-"));
  ...
  it("scans project-relative paths from an explicit changed-files file", async () => {
    const rootDirectory = fs.mkdtempSync(path.join(os.tmpdir(), "harness-doctor-changed-files-"));
    tempDirectories.push(rootDirectory);
    const webDirectory = path.join(rootDirectory, "apps", "web");
    const adminDirectory = path.join(rootDirectory, "apps", "admin");
    writePackageJson(rootDirectory, {
