...
describe("GitHub Action contract", () => {
  ...
  it("renders and posts the sticky comment before restoring scan failure", () => {
    ...
    const failIndex = actionYaml.indexOf("- name: Fail if Harness Doctor found blocking issues");
    ...
    expect(commentStep).toContain("<!-- harness-doctor:summary -->");
  ...
  it("non-blocking input makes the fail gate always exit 0", () => {
    ...
    const failStep = normalizeWhitespace(
      extractStep(actionYaml, "- name: Fail if Harness Doctor found blocking issues"),
    );
