...
const setupFixture = (): GitHookFixture => {
  const root = mkdtempSync(path.join(tmpdir(), "harness-doctor-git-hook-"));
  return {
    projectRoot: root,
...
describe.skipIf(process.platform === "win32")("installHarnessDoctorGitHook", () => {
  ...
  it("creates a dependency-free non-blocking pre-commit hook without a managed runner", () => {
    const result = installHarnessDoctorGitHook({
    ...
    expect(hookContent).toContain("pnpm dlx @andypai/harness-doctor@latest --staged --fail-on warning");
    expect(hookContent).toContain("npx --yes @andypai/harness-doctor@latest --staged --fail-on warning");
    expect(hookContent).toContain("Want them fixed?");
    expect(hookContent).not.toContain("Stop commit");
    expect(hookContent).not.toContain(".harness-doctor/hooks/pre-commit");
  ...
  it("preserves existing hook content", () => {
    ...
    expect(hookContent.startsWith("#!/bin/sh\n\n# harness-doctor hook start")).toBe(true);
    expect(hookContent).toContain("npm test\n");
  });

  it("updates the Harness Doctor block instead of duplicating it", () => {
    installHarnessDoctorGitHook({ hookPath: fixture.hookPath, projectRoot: fixture.projectRoot });
    installHarnessDoctorGitHook({ hookPath: fixture.hookPath, projectRoot: fixture.projectRoot });

    const hookContent = readHook(fixture.hookPath);
    const managedBlockMatches = hookContent.match(/# harness-doctor hook start/g) ?? [];
  ...
  it("replaces the legacy managed-runner launcher block", () => {
    const legacyRunnerPath = path.join(fixture.projectRoot, ".harness-doctor/hooks/pre-commit");
    mkdirSync(path.dirname(fixture.hookPath), { recursive: true });
    ...
    writeFileSync(
      ...
      [
        "#!/bin/sh",
        "",
        "# harness-doctor hook launcher start",
        'if [ -f ".harness-doctor/hooks/pre-commit" ]; then',
        '  sh ".harness-doctor/hooks/pre-commit"',
        "fi",
        "# harness-doctor hook launcher end",
        "",
      ].join("\n"),
    );
    ...
    const hookContent = readHook(fixture.hookPath);
    expect(hookContent).toContain("# harness-doctor hook start");
    expect(hookContent).toContain("harness-doctor --staged --fail-on warning");
  ...
  it("uses Husky when the project has Husky installed", () => {
    ...
    expect(readHook(target.hookPath)).toContain("harness-doctor --staged --fail-on warning");
  });
  ...
  it("updates package.json simple-git-hooks config", () => {
    ...
    expect(preCommit.match(/harness_doctor_output=\$\(mktemp/g)).toHaveLength(1);
    expect(preCommit).toContain("harness-doctor --staged --fail-on warning");
  });
  ...
  it("creates lefthook config for lefthook projects", () => {
    ...
    expect(configContent.match(/harness_doctor_output=\$\(mktemp/g)).toHaveLength(1);
    expect(configContent).toContain("run: harness_doctor_output=$(mktemp");
    expect(configContent).toContain("Harness Doctor found staged regressions.");
  });

  it("merges Harness Doctor into an existing Lefthook pre-commit section", () => {
    execFileSync("git", ["init"], { cwd: fixture.projectRoot, stdio: "ignore" });
  ...
  it("updates pre-commit config with a local hook", () => {
    ...
    expect(configContent.match(/id: harness-doctor/g)).toHaveLength(1);
    expect(configContent).toContain("entry: sh -c 'harness_doctor_output=$(mktemp");
  });
  ...
  it("updates Overcommit config", () => {
    ...
    expect(configContent.match(/HarnessDoctor/g)).toHaveLength(1);
    expect(configContent).toContain("command: ['sh', '-c', 'harness_doctor_output=$(mktemp");
  });

  it("merges Harness Doctor into an existing Overcommit PreCommit section", () => {
    execFileSync("git", ["init"], { cwd: fixture.projectRoot, stdio: "ignore" });
    const configPath = path.join(fixture.projectRoot, ".overcommit.yml");
    writeFileSync(
      configPath,
    ...
    expect(configContent).toContain("  HarnessDoctor:\n    enabled: true");
  ...
  it("updates npm pre-commit package config", () => {
    ...
    expect(packageJson["pre-commit"][1]).toContain("harness-doctor --staged --fail-on warning");
  });
  ...
  it("runs through a configured hooks directory during a real git commit", () => {
    ...
    execFileSync("git", ["config", "user.name", "Harness Doctor"], { cwd: fixture.projectRoot });
    ...
    const localBinaryPath = path.join(fixture.projectRoot, "node_modules/.bin/harness-doctor");
  ...
  it("prints a minimal non-invasive prompt during a real git commit", () => {
    ...
    expect(commitResult.stderr).toContain("Harness Doctor found staged regressions.");
    expect(commitResult.stderr).toContain(
      "Run harness-doctor --staged --fail-on warning to inspect.",
    );
    expect(commitResult.stderr).toContain(
