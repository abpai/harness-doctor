...
describe.skipIf(process.platform === "win32")("installHarnessDoctorGitHook", () => {
  ...
  it("creates a dependency-free non-blocking pre-commit hook without a managed runner", () => {
    ...
    expect(hookContent).toContain("pnpm dlx harness-doctor@latest --staged --fail-on warning");
    expect(hookContent).toContain("npx --yes harness-doctor@latest --staged --fail-on warning");
