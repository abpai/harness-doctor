...
import { GitBaseBranchInvalid, HarnessDoctorError } from "@harness-doctor/core";
...
describe("handleError", () => {
  ...
  it("builds a prefilled GitHub issue URL with redacted OTel context", () => {
    ...
    expect(body).toContain("HARNESS_DOCTOR_OTLP_ENDPOINT configured: yes");
    expect(body).toContain("HARNESS_DOCTOR_OTLP_AUTH_HEADER configured: yes (value redacted)");
  ...
  it("suggests Discord when printing an error", () => {
    ...
    expect(errorMessages.join("\n")).toContain(
      "You can also ask for help in Discord: https://harness.doctor/discord",
    );
...
describe("handleUserError", () => {
  ...
  it("prints just the message — no crash framing, issue link, Discord, or Sentry reference", () => {
    ...
    const userError = new HarnessDoctorError({
