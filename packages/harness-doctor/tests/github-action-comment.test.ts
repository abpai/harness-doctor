...
import type { Diagnostic, JsonReportProjectEntry, ProjectInfo } from "@harness-doctor/core";
...
const setupTempDirectory = (): string => {
  const directory = fs.mkdtempSync(path.join(os.tmpdir(), "harness-doctor-comment-"));
...
describe("render-github-action-comment", () => {
  ...
  it("renders a sticky markdown summary and action outputs from a JSON report", () => {
    ...
    expect(comment).toContain("<!-- harness-doctor:summary -->");
    expect(comment).toContain("<strong>Harness Doctor</strong>");
    expect(comment).toContain('<a href="https://harness.doctor">');
    expect(comment).toContain(
      "| 81 / 100 (Good) | 2 | 2 | 0 | 2 | 2 files changed on `feature` vs. `main` |",
    );
    expect(comment).toContain("`harness-doctor/no-derived-state`");
    expect(comment).toContain("[View workflow run]");
    expect(comment).toContain("Generated by [Harness Doctor]");
    expect(comment).toContain("https://github.com/abpai/harness-doctor");
  ...
  it("renders completed scans with no findings as a clean result", () => {
    ...
    expect(comment).toContain("No Harness Doctor issues found in this scan.");
    expect(comment).toContain(
  ...
  it("renders no-scan diff reports without a metrics table", () => {
    ...
    expect(comment).toContain(
      "Harness Doctor found 2 files changed in this pull request, but none matched the files covered by its enabled checks.",
    );
    ...
    const { comment, outputs } = runRenderer(
      buildReport({
        ...
        error: {
          ...
          message: "harness-doctor exited with status 1 before producing a JSON report.",
          chain: [],
        },
      }),
    );

    expect(comment).toContain("Harness Doctor could not complete this scan.");
    expect(comment).toContain("<strong>Harness Doctor</strong>");
    expect(comment).toContain("harness-doctor exited with status 1");
    expect(comment).toContain("[Report this bug]");
    expect(comment).toContain("issues/new?");
    expect(comment).toContain("Harness+Doctor+Action+failed");
    expect(comment).toContain("Generated by [Harness Doctor]");
    expect(comment).toContain("https://github.com/abpai/harness-doctor");
    expect(outputs).toContain("score=");
