...
describe("instrument config resolvers", () => {
  ...
  describe("resolveSentryRelease", () => {
    it("defaults to the `harness-doctor@<version>` form", () => {
      expect(resolveSentryRelease()).toMatch(/^harness-doctor@/);
    });

    it("honors the SENTRY_RELEASE override", () => {
      process.env.SENTRY_RELEASE = "harness-doctor@9.9.9";
      expect(resolveSentryRelease()).toBe("harness-doctor@9.9.9");
    });
  });
