...
// the spied module.
vi.mock("../src/cli/utils/install-harness-doctor.js", () => ({
  runInstallHarnessDoctor: vi.fn(),
}));
...
describe("installAction (Commander action wrapper)", () => {
  ...
  beforeEach(() => {
    ...
    process.exitCode = 0;
    vi.mocked(runInstallHarnessDoctor).mockReset();
  });
  ...
  // (exposed as `options.cwd`) into `runInstallHarnessDoctor({ projectRoot })`.
  // Typos in this thin wire-up (e.g. `projectRoot: options.dryRun`) would
  // silently send the wrong directory to the installer.
  it("forwards --cwd as projectRoot to runInstallHarnessDoctor", async () => {
    await installAction({ cwd: "/tmp/some-project", yes: true, dryRun: true });
    expect(runInstallHarnessDoctor).toHaveBeenCalledTimes(1);
    expect(runInstallHarnessDoctor).toHaveBeenCalledWith({
      yes: true,
      dryRun: true,
  ...
  it("forwards --agent-hooks to runInstallHarnessDoctor", async () => {
  ...
  it("delegates thrown errors to handleError without re-throwing", async () => {
    vi.mocked(runInstallHarnessDoctor).mockRejectedValueOnce(new Error("boom"));
