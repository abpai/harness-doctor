...
import { runInstallHarnessDoctor } from "../src/cli/utils/install-harness-doctor.js";
import type { InstallHarnessDoctorDependencyRunnerInput } from "../src/cli/utils/install-harness-doctor.js";
import { setSpinnerSilent } from "../src/cli/utils/spinner.js";
import { silenceConsoleForTest } from "./helpers/silence-console.js";

interface InstallHarnessDoctorFixture {
  projectRoot: string;
  sourceDir: string;
  cleanup: () => void;
}

const setupFixture = (): InstallHarnessDoctorFixture => {
  const root = mkdtempSync(path.join(tmpdir(), "harness-doctor-install-"));
...
let dependencyInstallCalls: InstallHarnessDoctorDependencyRunnerInput[] = [];

const installDependencyForTest: NonNullable<
  Parameters<typeof runInstallHarnessDoctor>[0]
...
const runInstallHarnessDoctorForTest = (
  options: NonNullable<Parameters<typeof runInstallHarnessDoctor>[0]>,
) =>
  runInstallHarnessDoctor({
    installDependencyRunner: installDependencyForTest,
    ...options,
  });

interface RunInteractiveInstallHarnessDoctorForTestOptions {
...
const runInteractiveInstallHarnessDoctorForTest = async (
  options: RunInteractiveInstallHarnessDoctorForTestOptions,
): Promise<void> => {
  ...
  const prompt: NonNullable<Parameters<typeof runInstallHarnessDoctor>[0]>["prompt"] = async <
  ...
  try {
    ...
    await runInstallHarnessDoctorForTest({
...
describe("runInstallHarnessDoctor", () => {
  let fixture: InstallHarnessDoctorFixture;
  ...
  it("throws when SKILL.md exists but has no parseable frontmatter (H1 silent-success regression guard)", async () => {
    ...
    await expect(
      runInstallHarnessDoctorForTest({
        yes: true,
        sourceDir: fixture.sourceDir,
        projectRoot: fixture.projectRoot,
        detectedAgents: ["cursor"],
  ...
  it("adds doctor package setup when package.json exists and setup is missing", async () => {
    ...
    expect(readFixturePackageJson(fixture.projectRoot).scripts).toEqual({
      test: "vite-plus test",
      doctor: "bunx --bun @andypai/harness-doctor@latest",
    });
  ...
  it("installs harness-doctor with the detected package manager", async () => {
    writeValidSkill(fixture.sourceDir);
    writePackageJson(fixture.projectRoot, {
  ...
  it("adds a harness-doctor package script when doctor is already taken", async () => {
    writeValidSkill(fixture.sourceDir);
    writePackageJson(fixture.projectRoot, {
      scripts: {
        doctor: "vitest --run",
  ...
  it("does not overwrite an existing harness-doctor dependency", async () => {
    writeValidSkill(fixture.sourceDir);
    writePackageJson(fixture.projectRoot, {
      devDependencies: {
  ...
  it("installs the skill into .factory/skills for the droid agent (upstream agent-install@0.0.3)", async () => {
    ...
    expect(
      existsSync(path.join(fixture.projectRoot, ".factory/skills/harness-doctor/SKILL.md")),
    ).toBe(true);
  });
  ...
  it("prompts once for optional setup and installs only selected options", async () => {
    ...
    const hookPath = path.join(fixture.projectRoot, ".git/hooks/pre-commit");
    const workflowPath = path.join(fixture.projectRoot, ".github/workflows/harness-doctor.yml");

    await runInteractiveInstallHarnessDoctorForTest({
      sourceDir: fixture.sourceDir,
      projectRoot: fixture.projectRoot,
      gitHookPath: hookPath,
    ...
    expect(workflowContent).toContain("name: Harness Doctor");
    ...
    expect(workflowContent).toContain("abpai/harness-doctor@main");
  ...
  it("honors selected setup actions when skip is selected with another option", async () => {
    ...
    expect(existsSync(hookPath)).toBe(false);
    expect(existsSync(path.join(fixture.projectRoot, ".cursor/hooks.json"))).toBe(false);
    expect(readFileSync(workflowPath, "utf8")).toContain("abpai/harness-doctor@main");
  });
