...
import { clearConfigCache, loadConfigWithSource } from "@harness-doctor/core";
import type { HarnessDoctorConfig } from "@harness-doctor/core";

const loadConfig = async (rootDirectory: string): Promise<HarnessDoctorConfig | null> => {
  clearConfigCache();
  return (await loadConfigWithSource(rootDirectory))?.config ?? null;
};

const tempRootDirectory = fs.mkdtempSync(path.join(os.tmpdir(), "harness-doctor-config-test-"));
...
describe("loadConfig", () => {
  ...
  describe("JSONC tolerance", () => {
    it("parses comments and trailing commas in doctor.config.json", async () => {
      ...
      expect(config).toEqual({ rules: { "harness-doctor/no-danger": "off" } });
    });
  });

  describe("package.json harnessDoctor key", () => {
    ...
    it("loads config from package.json harnessDoctor key", async () => {
  ...
  describe("config file takes precedence", () => {
    ...
    beforeAll(() => {
      ...
      fs.writeFileSync(
        ...
        JSON.stringify({
          name: "test",
          harnessDoctor: { ignore: { rules: ["from-package-json"] } },
        }),
      );
    });
  ...
  describe("invalid config", () => {
    ...
    it("falls through to package.json when config file has malformed JSON", async () => {
      ...
      fs.writeFileSync(
        ...
        JSON.stringify({
          name: "test",
          harnessDoctor: { ignore: { rules: ["from-fallback"] } },
        }),
      );
    ...
    it("falls through to package.json when config file is not an object", async () => {
      ...
      fs.writeFileSync(
        ...
        JSON.stringify({
          name: "test",
          harnessDoctor: { ignore: { rules: ["from-non-object-fallback"] } },
        }),
      );
    ...
    it("ignores non-object harnessDoctor key in package.json", async () => {
      ...
      fs.writeFileSync(
        ...
        JSON.stringify({ name: "test", harnessDoctor: "not-an-object" }),
      );
  ...
  describe("ancestor config inheritance", () => {
    ...
    it("finds config from package.json harnessDoctor key in ancestor", async () => {
      ...
      fs.writeFileSync(
        ...
        JSON.stringify({
          name: "monorepo",
          harnessDoctor: { customRulesOnly: true },
        }),
      );
