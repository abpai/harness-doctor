...
const reportRuleNotFound = (ruleQuery: string): void => {
  ...
  logger.dim("  Run `harness-doctor rules list` to see every available rule.");
  process.exitCode = 1;
};
...
interface AppliedConfigChange {
  ...
  readonly nextConfig: HarnessDoctorConfig;
  readonly written: boolean;
}

const applyConfigChange = async (
  options: RulesCwdOptions,
  change: (config: HarnessDoctorConfig) => HarnessDoctorConfig,
...
const reportManualEdit = (target: RuleConfigTarget, nextConfig: HarnessDoctorConfig): void => {
      ...
      JSON.stringify(
        {
          ...
          learnMoreUrl: buildRuleDocsUrl("harness-doctor", entry.id),
        },
        null,
        2,
      ),
...
export const rulesEnableAction = async (
  ...
  if (severity === "off") {
    logger.error("`enable` cannot set a rule to off. Use `harness-doctor rules disable` instead.");
    process.exitCode = 1;
    return;
  }
  await setRuleSeverityAndReport(entry, severity, options);
