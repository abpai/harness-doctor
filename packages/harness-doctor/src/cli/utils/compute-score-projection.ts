import { calculateLocalScore, TOP_ERRORS_DISPLAY_COUNT } from "@harness-doctor/core";
import type { Diagnostic, ScoreResult } from "@harness-doctor/core";
...
export const computeProjectedScore = (
  ...
  const potentialScore = calculateLocalScore(remainingDiagnostics, rescoreSource.length);
  if (potentialScore.score <= currentScore.score) return null;
