 ...
 * `harness-doctor` run is a first-class trace with timing and the run snapshot as
...
export const withSentryRunSpan = <T>(run: (rootSpan: SentryRootSpan) => Promise<T>): Promise<T> => {
  ...
  return Sentry.startSpan(
    { name: `harness-doctor ${command}`, op: "cli.inspect", attributes: toSpanAttributes(tags) },
    (rootSpan) => {
