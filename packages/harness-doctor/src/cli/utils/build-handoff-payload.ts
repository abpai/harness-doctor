import { TOP_ERRORS_DISPLAY_COUNT } from "@harness-doctor/core";
...
export const buildHandoffPayload = (input: HandoffPayloadInput): string => {
  ...
  const lines: string[] = [
    `Fix the top ${topGroups.length} Harness Doctor ${topGroups.length === 1 ? "issue" : "issues"} in ${input.projectName} on this pass — leave the rest for a follow-up.`,
    "",
  ];
  ...
  lines.push(
    ...
    "",
    "Verify against the real thing, don't assume: confirm each change matches the canonical fix recipe you fetched for that rule, then re-run `bunx --bun @andypai/harness-doctor@latest --verbose` and check the issue is actually gone against the real tool before moving on.",
    "",
