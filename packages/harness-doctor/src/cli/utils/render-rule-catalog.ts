import { buildRuleDocsUrl, highlighter } from "@harness-doctor/core";
import type { RuleSeverityOverride } from "@harness-doctor/core";
...
export const renderRuleExplanation = (row: RenderedRuleRow): string => {
  ...
  lines.push(highlighter.dim(`  harness-doctor rules disable ${entry.key}`));
  lines.push(highlighter.dim(`  harness-doctor rules enable ${entry.key} --severity error`));
  lines.push(highlighter.dim(`  harness-doctor rules set ${entry.key} warn`));

  lines.push("");
  lines.push(highlighter.bold("Learn more"));
  lines.push(highlighter.dim(`  ${buildRuleDocsUrl("harness-doctor", entry.id)}`));
