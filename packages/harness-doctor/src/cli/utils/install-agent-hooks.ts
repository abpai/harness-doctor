...
const CLAUDE_HOOK_RELATIVE_PATH = ".claude/hooks/harness-doctor.sh";
const CLAUDE_HOOK_COMMAND = 'sh "$CLAUDE_PROJECT_DIR/.claude/hooks/harness-doctor.sh"';
const CURSOR_HOOKS_RELATIVE_PATH = ".cursor/hooks.json";
const CURSOR_HOOK_RELATIVE_PATH = ".cursor/hooks/harness-doctor.sh";
...
const buildAgentHookScript = (): string =>
  [
    ...
    "set -u",
    "",
    'input_file=$(mktemp "${TMPDIR:-/tmp}/harness-doctor-agent-hook.XXXXXX")',
    'output_file=$(mktemp "${TMPDIR:-/tmp}/harness-doctor-agent-hook-output.XXXXXX")',
    ...
    "    ./node_modules/.bin/harness-doctor --verbose --diff --fail-on warning --no-score",
    "    return",
    "  fi",
    "",
    "  if command -v harness-doctor >/dev/null 2>&1; then",
    "    harness-doctor --verbose --diff --fail-on warning --no-score",
    "    return",
    "  fi",
    "",
    ...
    "    pnpm dlx @andypai/harness-doctor@latest --verbose --diff --fail-on warning --no-score",
    "    return",
    "  fi",
    "",
    "  if command -v npx >/dev/null 2>&1; then",
    "    npx --yes @andypai/harness-doctor@latest --verbose --diff --fail-on warning --no-score",
    "    return",
    "  fi",
    "",
    "  printf '%s\\n' 'harness-doctor: command not found; skipping agent hook scan.'",
    "  return 0",
    "}",
    "",
    "if ! should_scan; then",
    ...
    "fi",
    "",
    'if run_harness_doctor > "$output_file" 2>&1; then',
    "  exit 0",
    "fi",
    "",
    ...
    "const message = `Harness Doctor found issues in the changed files. Review this output and fix the regressions before finishing. For confirmed issues that cannot be fixed now, create GitHub issues with the rule, file/line, confidence, impact, and proposed fix.\\n\\n${scanOutput}`;",
...
export const installHarnessDoctorAgentHooks = (
