...
import { highlighter, SKILL_NAME } from "@harness-doctor/core";
...
import { installHarnessDoctorAgentHooks } from "./install-agent-hooks.js";
import { isRecord, readPackageJson } from "./git-hook-shared.js";
import { GitHookKind, type GitHookTarget } from "./git-hook-types.js";
import { detectGitHookTarget, installHarnessDoctorGitHook } from "./install-git-hook.js";
...
export interface InstallHarnessDoctorDependencyRunnerInput {
  readonly command: string;
  readonly args: ReadonlyArray<string>;
  readonly cwd: string;
}

interface InstallHarnessDoctorDependencyOptions {
  readonly projectRoot: string;
  readonly runner?: (input: InstallHarnessDoctorDependencyRunnerInput) => void | Promise<void>;
}

interface InstallHarnessDoctorDependencyResult {
...
const buildInstallCommand = (projectRoot: string): InstallHarnessDoctorDependencyRunnerInput => {
...
const defaultInstallDependencyRunner = (input: InstallHarnessDoctorDependencyRunnerInput): void => {
  ...
  execFileSync(input.command, [...input.args], {
    ...
    env: { ...process.env, HARNESS_DOCTOR_INSTALL: "1" },
...
const formatInstallCommand = (input: InstallHarnessDoctorDependencyRunnerInput): string =>
  [input.command, ...input.args].join(" ");

const installHarnessDoctorDependency = async (
  options: InstallHarnessDoctorDependencyOptions,
): Promise<InstallHarnessDoctorDependencyResult> => {
...
const formatGitHookInstallMessage = (
  hookResult: ReturnType<typeof installHarnessDoctorGitHook>,
): string => {
  if (CONFIG_ONLY_GIT_HOOK_KINDS.has(hookResult.kind)) {
    return `Harness Doctor pre-commit config ${hookResult.status} at ${hookResult.hookPath}. Run your hook manager's install command if hooks are not already installed.`;
  }
  return `Harness Doctor pre-commit hook ${hookResult.status} at ${hookResult.hookPath}.`;
};
...
const formatDoctorScriptInstallMessage = (
  ...
  if (scriptResult.scriptStatus === "created") {
    ...
    messages.push("Skipped package script: doctor and harness-doctor are already taken.");
  } else if (scriptResult.scriptReason === "doctor-script-taken") {
    messages.push("Skipped package script: doctor and harness-doctor scripts already exist.");
...
const formatDependencyInstallMessage = (result: InstallHarnessDoctorDependencyResult): string => {
  if (result.dependencyStatus === "created") {
    return "Installed dev dependency: harness-doctor.";
  }
  if (result.dependencyStatus === "existing") {
    return "Harness Doctor dependency already exists.";
  }
  ...
  if (result.dependencyReason === "trust-policy-blocked") {
    ...
    return `Skipped local install: your package manager's supply-chain trust policy blocked a dependency (not a compromise — beta packages trip this). Harness Doctor still works via \`bunx --bun @andypai/harness-doctor\`. To add it locally: ${installCommand}`;
  }
...
const installHarnessDoctorPackageSetup = async (
  projectRoot: string,
  dependencyRunner?: (input: InstallHarnessDoctorDependencyRunnerInput) => void | Promise<void>,
): Promise<InstallHarnessDoctorDependencyResult> => {
  const scriptSpinner = spinner("Installing Harness Doctor package script...").start();
  try {
    const scriptResult = installDoctorScript({ projectRoot });
    scriptSpinner.succeed(formatDoctorScriptInstallMessage(scriptResult));
  } catch (error) {
    scriptSpinner.fail("Failed to install Harness Doctor package script.");
    throw error;
  }

  const dependencySpinner = spinner("Installing Harness Doctor package...").start();
  try {
    const dependencyResult = await installHarnessDoctorDependency({
      projectRoot,
    ...
    dependencySpinner.fail("Failed to install Harness Doctor package.");
    throw error;
  }
};

interface InstallHarnessDoctorOptions {
  yes?: boolean;
  dryRun?: boolean;
  agentHooks?: boolean;
  ...
  installDependencyRunner?: (
    input: InstallHarnessDoctorDependencyRunnerInput,
  ) => void | Promise<void>;
...
// Discovers skills that ship alongside the primary `harness-doctor` skill
...
const buildWorkflowContent = (): string =>
  [
    ...
    "",
    "concurrency:",
    "  group: harness-doctor-${{ github.event.pull_request.number || github.ref }}",
...
export const runInstallHarnessDoctor = async (
  options: InstallHarnessDoctorOptions = {},
): Promise<void> => {
  ...
  const workflowTargetPath = path.join(workflowsDirectory, "harness-doctor.yml");
          ...
          await prompt<"setupOptions">(
            {
              type: "multiselect",
              name: "setupOptions",
              message: "Select additional Harness Doctor setup:",
  ...
  if (options.dryRun) {
    ...
    logger.dim("  Package script: doctor (or harness-doctor if doctor exists)");
    logger.dim("  Dev dependency: harness-doctor");
  ...
  if (shouldInstallGitHook && gitHookTarget !== null && gitHookTarget !== undefined) {
    ...
    try {
      const hookResult = installHarnessDoctorGitHook({
      ...
      hookSpinner.fail("Failed to install Harness Doctor pre-commit hook.");
      throw error;
    }
  }

  if (shouldInstallAgentHooks) {
    const hookSpinner = spinner("Installing Harness Doctor agent hooks...").start();
    try {
      const hookResult = installHarnessDoctorAgentHooks({
        projectRoot,
        agents: selectedAgents,
      });
      if (hookResult.installedAgents.length === 0) {
        ...
        hookSpinner.succeed(
          `Harness Doctor agent hooks installed for ${hookResult.installedAgents.map((agent) => getSkillAgentConfig(agent).displayName).join(", ")}.`,
        );
      }
    } catch (error) {
      hookSpinner.fail("Failed to install Harness Doctor agent hooks.");
      throw error;
    }
  }
