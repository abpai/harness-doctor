import { setColorEnabled } from "@harness-doctor/core";

/**
 * Resolve an explicit color preference from `--color` / `--no-color` or the
 * app-specific `HARNESS_DOCTOR_NO_COLOR` / `HARNESS_DOCTOR_FORCE_COLOR` env vars
...
export const applyColorPreference = (
  ...
  if (enabled === undefined) {
    ...
    if (env.HARNESS_DOCTOR_NO_COLOR) enabled = false;
    else if (env.HARNESS_DOCTOR_FORCE_COLOR) enabled = true;
  }
