...
const DOCTOR_SCRIPT_COMMAND = "npx harness-doctor@latest";
export const DOCTOR_PACKAGE_NAME = "harness-doctor";
...
const isHarnessDoctorScriptCommand = (value: unknown): boolean =>
  typeof value === "string" && /\bharness-doctor\b/.test(value);
...
export const hasDoctorScript = (projectRoot: string): boolean => {
  ...
  return (
    isHarnessDoctorScriptCommand(scripts[DOCTOR_SCRIPT_NAME]) ||
    isHarnessDoctorScriptCommand(scripts[FALLBACK_DOCTOR_SCRIPT_NAME])
  );
};
...
export const installDoctorScript = (
  ...
  const scriptTarget = (() => {
    ...
    if (isHarnessDoctorScriptCommand(scriptRecord[DOCTOR_SCRIPT_NAME])) {
      return {
    ...
    if (!Object.hasOwn(scriptRecord, DOCTOR_SCRIPT_NAME)) {
      if (isHarnessDoctorScriptCommand(scriptRecord[FALLBACK_DOCTOR_SCRIPT_NAME])) {
        return {
