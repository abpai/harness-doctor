...
const isOtlpExportConfigured = (): boolean =>
  Boolean(process.env.HARNESS_DOCTOR_OTLP_ENDPOINT) &&
  Boolean(process.env.HARNESS_DOCTOR_OTLP_AUTH_HEADER);
 ...
 * precedence:
 *
 * 1. **User OTLP backend** (`HARNESS_DOCTOR_OTLP_*` set) wins; we additionally
