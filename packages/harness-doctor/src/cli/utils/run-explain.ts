import { highlighter, toRelativePath } from "@harness-doctor/core";
import { cliLogger as logger } from "./cli-logger.js";
import { inspect } from "../../inspect.js";
import type { Diagnostic, InspectOptions, HarnessDoctorConfig } from "@harness-doctor/core";
...
export const runExplain = async (
  ...
  if (matchingDiagnostics.length === 0) {
    logger.log(`No harness-doctor diagnostics at ${filePath}:${line}.`);
    return;
  }
  ...
  for (const diagnostic of matchingDiagnostics) {
    ...
    if (diagnostic.suppressionHint) {
      ...
      logger.dim(
        "  No nearby harness-doctor-disable-next-line comment was detected — add one immediately above this line to suppress.",
      );
    }
    logger.break();
  }
