...
import { isHarnessDoctorError } from "@harness-doctor/core";
...
export const reportErrorToSentry = async (error: unknown): Promise<string | undefined> => {
  ...
  try {
    ...
    // command + the tagged `HarnessDoctorError` reason (or the legacy thrown
    // class name), complementing the captured Sentry issue.
    let reason = "unknown";
    if (isHarnessDoctorError(error)) reason = error.reason._tag;
