...
${formatExampleLines([
  ...
  ["harness-doctor --explain src/App.tsx:42", "explain why a rule fired there"],
  ["harness-doctor install", "set up the agent skill and git hook"],
])}
...
${highlighter.dim("Examples:")}
${formatExampleLines([
  ["harness-doctor install", "interactive setup"],
  ["harness-doctor install --yes", "non-interactive; all detected agents"],
  ["harness-doctor install --dry-run", "preview without writing files"],
  ["harness-doctor install --agent-hooks", "also install native agent hooks"],
])}
...
program
  .command("install")
  .alias("setup")
  .description("Install the harness-doctor skill into your coding agents and optional git hook")
...
// `harness-doctor . | head`), Node throws an uncaught EPIPE on the next
...
// trailing tokens like `harness-doctor help --no-color`) so the choice
