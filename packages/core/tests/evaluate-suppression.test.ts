...
import { evaluateSuppression } from "@harness-doctor/core";
...
describe("evaluateSuppression near-miss hints", () => {
  it("marks same-line disable-line comments as suppressed", () => {
    const lines = linesOf(
      `const x = 1; // harness-doctor-disable-line harness-doctor/no-derived-state-effect\n`,
    );
    const result = evaluateSuppression(lines, 0, "harness-doctor/no-derived-state-effect");
  ...
  it("marks stacked disable-next-line comments as suppressed when each rule is listed separately", () => {
    const lines = linesOf(
      `// harness-doctor-disable-next-line harness-doctor/no-derived-state-effect\n` +
        `// harness-doctor-disable-next-line harness-doctor/no-fetch-in-effect\n` +
        `const x = 1;\n`,
    );
    expect(evaluateSuppression(lines, 2, "harness-doctor/no-derived-state-effect")).toEqual({
      isSuppressed: true,
      nearMissHint: null,
    });
    expect(evaluateSuppression(lines, 2, "harness-doctor/no-fetch-in-effect")).toEqual({
      isSuppressed: true,
      nearMissHint: null,
    });
  });
  ...
  it("returns null when no nearby disable-next-line comment exists", () => {
    ...
    expect(nearMissHintFor(lines, 1, "harness-doctor/no-derived-state-effect")).toBeNull();
  });
  ...
  it("emits a wrong-rule hint when an adjacent comment lists different rules", () => {
    ...
    const hint = nearMissHintFor(lines, 1, "harness-doctor/no-derived-state-effect");
    expect(hint).not.toBeNull();
    expect(hint).toContain("comma form");
    expect(hint).toContain("harness-doctor/no-derived-state-effect");
  });
  ...
  it("emits a gap-code hint when a code line breaks the chain to the matching comment", () => {
    ...
    const hint = nearMissHintFor(lines, 2, "harness-doctor/no-derived-state-effect");
    expect(hint).not.toBeNull();
  ...
  it("returns null when a description follows the matching rule list", () => {
    const lines = linesOf(
      `// harness-doctor-disable-next-line harness-doctor/no-derived-state-effect -- intentional fixture state mirror\nconst x = 1;\n`,
    );
    const result = evaluateSuppression(lines, 1, "harness-doctor/no-derived-state-effect");
    expect(result).toEqual({ isSuppressed: true, nearMissHint: null });
  });
});
