...
const TRACER_PROJECT_NAME = "harness-doctor";

const OTEL_ENDPOINT = Config.string("HARNESS_DOCTOR_OTLP_ENDPOINT").pipe(Config.option);
const OTEL_AUTH_HEADER = Config.redacted("HARNESS_DOCTOR_OTLP_AUTH_HEADER").pipe(Config.option);
 ...
 *
 *   HARNESS_DOCTOR_OTLP_ENDPOINT      e.g. https://api.axiom.co
 *   HARNESS_DOCTOR_OTLP_AUTH_HEADER   e.g. "Bearer <token>"
 *
 * Both env vars are required to enable export — if either is
 * missing, the layer is a no-op (matches the pattern from
 * `harness-doctor-evals/src/Observability.ts`, where the equivalent
