...
const toWorkspacePackages = (directories: string[]): WorkspacePackage[] => {
...
const listManifestWorkspacePackages = (rootDirectory: string): WorkspacePackage[] => {
  ...
  return toWorkspacePackages(directories);
};
...
// these and surface vendored packages the user never authored.
...
const discoverSubprojectsByFilesystem = (rootDirectory: string): WorkspacePackage[] => {
  const packages: WorkspacePackage[] = [];
  // HACK: stack + .pop() rather than queue + .shift() because Array.shift()
  // is O(n), which degraded this walk to O(n^2) on large trees. Result is a
  // depth-first visit order, which doesn't matter for the final list.
...
export const discoverSubprojects = (rootDirectory: string): WorkspacePackage[] => {
  ...
  return discoverSubprojectsByFilesystem(rootDirectory);
};
