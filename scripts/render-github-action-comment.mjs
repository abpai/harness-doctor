...
const MARKER = "<!-- harness-doctor:summary -->";
const BRAND_IMAGE_URL =
  "https://raw.githubusercontent.com/millionco/harness-doctor/main/packages/website/public/harness-doctor-og-banner.svg";
const BRAND_LINK_URL = "https://harness.doctor";
const BUG_REPORT_URL = "https://github.com/millionco/harness-doctor";
const BRAND_HEADER = `<a href="${BRAND_LINK_URL}"><img src="${BRAND_IMAGE_URL}" alt="Harness Doctor" width="244" /></a>`;
const BRAND_FOOTER = `Generated by [Harness Doctor](${BRAND_LINK_URL}). Questions? Contact founders@million.dev.`;
...
const buildBugReportUrl = (report) => {
  ...
  const message = report.error?.message ?? "Harness Doctor failed before completing the scan.";
  const bodyLines = [
    "Harness Doctor Action failed before completing a scan.",
    "",
    "### Error",
    "",
    `\`${message}\``,
    ...
    "### Context",
    "",
    `- Harness Doctor version: ${report.version ?? "unknown"}`,
    `- Mode: ${report.mode ?? "unknown"}`,
    runUrl ? `- Workflow run: ${runUrl}` : null,
  ].filter(Boolean);
  const parameters = new URLSearchParams({
    title: "Harness Doctor Action failed",
    body: bodyLines.join("\n"),
  });
...
const buildNoScanMessage = (report) => {
  ...
  if (report.mode === "diff") {
    ...
    if (changedFileCount === 0) {
      return "No changed files were found in this pull request, so Harness Doctor skipped the scan.";
    }
    return `Harness Doctor found ${pluralize(changedFileCount, "file")} changed in this pull request, but none matched the files covered by its enabled checks.`;
  }
  return "Harness Doctor did not find any files covered by its enabled checks.";
};
...
const buildStatusLine = (report) => {
  ...
  if (totalIssues === 0 && hasIncompleteChecks(report)) {
    return "No Harness Doctor issues were found, but some checks were incomplete.";
  }
  if (totalIssues === 0) return "No Harness Doctor issues found in this scan.";
  return `Harness Doctor found ${pluralize(totalIssues, "issue")} in ${pluralize(summary.affectedFileCount ?? 0, "file")}.`;
};
...
const buildErrorBody = (report) => {
  ...
  return renderLines([
    ...
    BRAND_HEADER,
    "",
    "Harness Doctor could not complete this scan.",
    "",
    `> ${message}`,
    "",
    runUrl ? `[View workflow run](${runUrl})` : "",
...
if (!report) {
  console.warn(
    "Harness Doctor: no scan report was found, so the summary comment was skipped. " +
